...
import { HexGridScreen, savedAngle } from "./grid.js"
import { startSetup } from "./setup/flow.js"

const app = createApp(document.getElementById("stage"))
const bootGame = angle => app.setScreen(HexGridScreen({ angle, onReset: bootSetup }))
const bootSetup = () => startSetup(app, store => bootGame(store.player.seedAngle))
const saved = savedAngle()
saved == null ? bootSetup() : bootGame(saved)
