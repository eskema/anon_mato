...
const bootGame = async world => {
  ...
  if (!fresh) {
    showLoad(`Replaying ${world.day} days…`)
  ...
  if (fresh) app.fadeIn()
  app.setScreen(HexGridScreen({ ...world, sim, onReset: resetAll }))
  hideLoad()
}
// RESET EVERYTHING means everything: the save AND the key. Setup skips its first
// screen while a key is remembered (see flow.js), so leaving it behind would
// make a reset land on the angle picker with the same identity still attached —
// which is not what the word says.
const resetAll = () => {
  forgetIdentity() // the key AND the cached profile — see identity.js
  bootSetup()
}
const bootSetup = () => {
