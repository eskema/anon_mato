...
// THE CLICK'S PULSE — a ring opening off a mark, fading as it goes and thinning
// as it opens, so it breathes rather than blinks: the sign that a click is
// waited for there (the compass's centre and reading, the intake's cursor in
// range). r0 → r1 over one breath; `alpha` scales it, `width` is the line at
// its start.
export function clickPulse(ctx, x, y, r0, r1, stroke, alpha = 1, width = 1.5, now = performance.now()) {
  if (alpha <= 0.002) return
  const u = (now % 1400) / 1400
  ctx.beginPath()
  ctx.arc(x, y, r0 + (r1 - r0) * u, 0, Math.PI * 2)
  ctx.strokeStyle = stroke
  ctx.globalAlpha = alpha * 0.6 * (1 - u)
  ctx.lineWidth = width * (1 - u * 0.75)
  ctx.stroke()
  ctx.globalAlpha = 1
}
