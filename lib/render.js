...
export function createRenderer(sim) {
  ...
  function cursorLabel(ctx, L, pointer, lines, tile) {
    ...
    // white leader lines from the cursor to EVERY corner of every line's
    // background box — a little fan. The boxes carry a matching white border
    // (below), so the lines flow into the corners. Drawn under the backgrounds.
    ctx.save()
    ctx.setLineDash([])
    ctx.strokeStyle = "#fff"
    ctx.globalAlpha = 1
