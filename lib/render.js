...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    const drawEnergyCube = (x, y, r) => {
      ...
      // pupil sat. While hovering a move it previews THAT move's cost instead, in
      // a lighter shade (the top status line still shows the real remaining).
      let mins = Math.round(freeEnergy)
      let faded = false
      if (!ui.pending && ui.hovered && ui.hoverPath) {
        mins = Math.round(sim.pathCost(ui.hoverPath))
        faded = true
      } else if (!ui.pending && ui.hovered && sim.isFrontier(ui.hovered) && sim.canScout(ui.hovered)) {
        mins = Math.round(sim.scoutCostAt(ui.hovered))
        faded = true
      }
      ctx.fillStyle = ink
