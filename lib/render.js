...
export function createRenderer(sim) {
  ...
  // the box were a solid facing the cursor. Drawn under the hover trail + boxes.
  ...
  function labelBox(ctx, lay, border = "#fff") {
    if (!lay) return
    const { lines, left, top, lineH, pad, widths, pointer } = lay
    ctx.textAlign = "left"
    ...
    lines.forEach((l, i) => {
      ...
      // border only the FRONT-facing edges — the away edges of the 3D slab hide
