...
export function createRenderer(sim) {
  ...
  // thin leader lines from the cursor to EVERY corner of every line box — a little
  // fan (default white; black for the menu). The boxes carry a matching border, so
  // the lines flow into the corners. Drawn UNDER the hover trail (and the boxes).
  ...
  function labelBox(ctx, lay, border = "#fff") {
    ...
    lines.forEach((l, i) => {
      ...
      ctx.strokeStyle = border // matching border with rounded corners, for the fan
      ctx.lineWidth = 2
