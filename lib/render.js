      ...
      for (const dlt of Hex.range(rCull)) {
        ...
        if (kind === "seam") {
          ...
          for (let d = 0; d < 6; d++) {
            ...
            const ve = nch != null ? (v0 + val(nch)) / 2 : v0
            const [wa, wb] = disp().edgeCorners[d]
            const A = cf[wa]
            const B = cf[wb]
            const grad = ctx.createLinearGradient(c.x, c.y, (A.x + B.x) / 2, (A.y + B.y) / 2)
            grad.addColorStop(0, grey(v0))
            grad.addColorStop(1, grey(ve))
            ctx.fillStyle = grad
            ctx.beginPath()
            ctx.moveTo(c.x, c.y)
            ctx.lineTo(A.x, A.y)
