    ...
    const dots = sim.reachableDots()
    {
      ...
      for (const dlt of Hex.range(rCull)) {
        ...
        if (kind === "seam") {
          ...
          // derived land (the pubkey inscription) shows as deeper tint steps —
          // monochrome v0; springs get a small well ring
          const tn = sim.typeNameAt(h)
          const extra = tn === "grove" ? 0.04 : tn === "marsh" ? 0.08 : tn === "crag" ? 0.13 : 0
          if (extra) fillHex(ctx, c, size, ink, extra, o.startDeg)
          if (tn === "spring") {
            ctx.strokeStyle = ink
            ctx.globalAlpha = 0.4
            ctx.lineWidth = 1.5
