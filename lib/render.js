...
// Covers the whole view — interior, seam and the neighbours' facing rows.
function buildEdges() {
  const seen = new Set()
  const out = []
  for (const [q, r] of Hex.range(VIEW_RING)) {
    for (let d = 0; d < 6; d++) {
      const nq = q + DIRS[d].q
      const nr = r + DIRS[d].r
...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    for (const h of Hex.range(VIEW_RING)) {
      ...
      if (sim.isDiscovered(h)) {
        ...
        if (sim.kindOf(h) === "seam") {
          // seam tiles (parent edges/vertices) read as rings: the border and the
          // inner hex are punched out in surface — only a band of fill remains.
          // The border grows INWARD (stroke fully inside the tile), so it never
          // clips the fills of the tiles across the seam.
          const lw = 1.5
          const inset = 1 - lw / (Math.sqrt(3) * size) // stroke's outer edge = tile edge
    ...
    for (const e of edges) {
      ...
      const ka = sim.kindOf(a)
      const kb = sim.kindOf(b)
      if (!ka && !kb) continue
      if (!sim.isDiscovered(a) && !sim.isDiscovered(b)) continue
      if (ka === "seam" && kb === "seam") continue // no borders within the seam
      if (ka === "seam" || kb === "seam") {
        // seam tiles own no border — the edge belongs to the tile across it,
        // drawn INWARD on that tile's side so nothing paints over the seam
        const t = ka === "seam" ? b : a
        const dir = ka === "seam" ? (e.dir + 3) % 6 : e.dir
        if (!(ka === "seam" ? kb : ka)) continue // seam against nothing: no line
        const inset = 1 - 1 / (Math.sqrt(3) * size) // stroke's outer edge = tile edge
        const c2 = hexToPixel(L, t[0], t[1], size)
        const cs2 = hexCorners(c2.x, c2.y, size * inset, o.startDeg)
        const [ca2, cb2] = disp().edgeCorners[dir]
        ctx.beginPath()
        ctx.moveTo(cs2[ca2].x, cs2[ca2].y)
        ctx.lineTo(cs2[cb2].x, cs2[cb2].y)
        ctx.stroke()
        continue
      }
    ...
    // border tiles, along their edges toward the seam — no way through there.
    if (v.tile.walls) {
      ctx.strokeStyle = ink
      ctx.globalAlpha = 0.7
      ctx.lineWidth = 3
