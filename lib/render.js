...
export function createRenderer(sim) {
  ...
  function cursorLabel(ctx, L, pointer, lines, tile) {
    ...
    // a white leader line from the cursor to the label, UNDER the backgrounds so
    // the rects cover its inner end (only the run across the gap shows)
    ctx.save()
    ctx.setLineDash([])
    ctx.strokeStyle = "#fff"
    ctx.globalAlpha = 1
    ...
    ctx.lineTo(left + boxW / 2, top + boxH / 2)
    ctx.stroke()
    ctx.restore()
    ctx.textAlign = "left"
