...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    const drawStack = (x, y, hasPlayer, hasNpc) => {
      boldHex(x, y, size) // the special-tile ring; background = the tile itself
      let prev = size
      if (hasPlayer) {
        thinSegs(x, y, prev, size * (2 / 3))
        castShadow(true)
        boldHex(x, y, size * (2 / 3), true)
        castShadow(false)
        prev = size * (2 / 3)
      }
      if (hasNpc) {
        ...
        boldY(x, y, size / 3) // …and the bold-Y seat stub
      }
    }
    ...
    if (!playerDrawn) {
      ...
      castShadow(true)
      boldHex(pc.x, pc.y, size * (2 / 3), true)
      castShadow(false)
