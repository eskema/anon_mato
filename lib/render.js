        ...
        if (kind === "seam") {
          ...
          if (chs != null) {
            ...
            if (node && node.discovered.size) {
              ...
              const ss = size / 9.5
              const isHomeSelf = h[0] === 0 && h[1] === 0 // the centre tile maps home ITSELF —
              // paint it as home is SEEN (identity colours), not its hidden ground terrain
              for (const dk of node.discovered) {
                const [lq, lr] = dk.split(",").map(Number)
                let pal
                if (isHomeSelf) {
                  const nch = sim.nibbleAt([lq, lr])
                  if (nch == null) continue
                  pal = nibbleColor([...nch].reduce((s, c2) => s + parseInt(c2, 16), 0) / nch.length)
                } else {
                  pal = BIOME_PAL[sim.typeNameAt([centre[0] + lq, centre[1] + lr])]
                }
                if (!pal) continue
                fillHex(
                  ctx,
