...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    function drawClock() {
      ...
      ctx.stroke()
      // 12 hour marks on the orbit (faint reference)
      ctx.lineCap = "butt"
      for (let i = 0; i < 12; i++) {
        const a = (i / 12) * TAU
        const long = i % 3 === 0
        ...
        const p1 = at(a, R)
        ctx.globalAlpha = long ? 0.28 : 0.14
      ...
      line(now + cost, 3, 0.2) // ghost: on to the run-out / after the hovered move
      line(now, 3, 0.85) // current time
