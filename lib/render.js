...
// for both. Pure black veil, deep (NIGHT_MAX), one radial gradient painted
// straight to the canvas: no sheets, no compositing, no bearings.
...
export function mapLight(ctx, L, at, size, { sunAlt, sunDeg, isNight, moon, black = false, douse = false, hole = null }) {
  if (!isNight) {
  ...
  // GENTLY (0.3, was 0.55: a full moon read almost as daylight): a bright
  // night is still night.
  const nightA = NIGHT_MAX * Math.pow(depth, 0.7) * (1 - 0.3 * moonLight)
  ...
  // DOUSED (2026-08-31) — dreaming, you carry nothing: the night lies flat and
  // the dream's own veil does the seeing. (Also the hook for any future
  // no-light state: exhaustion, a spent torch…)
  if (douse) {
    ctx.fillStyle = `rgba(0,0,0,${nightA})`
    ctx.fillRect(0, 0, L.w, L.h)
    ctx.globalAlpha = 1
    return () => 1 - nightA
  }
  if (!black) {
    // THE SETUP STAGE's plain round pool + sight vignette, as ever — the
    // game's lamp (black) is below.
    const grad = ctx.createRadialGradient(at.x, at.y, size * 1.1, at.x, at.y, size * 5)
    grad.addColorStop(0, rgba("#0d1021", nightA * 0.15))
    grad.addColorStop(1, rgba("#0d1021", nightA))
    ctx.fillStyle = grad
    ctx.fillRect(0, 0, L.w, L.h)
    const sightR = size * NIGHT_SIGHT_BASE
    if (edgeA > 0.01) {
  ...
  // THE LAMP — one round gradient at `at` (the caller hands the EYE point),
  // painted straight to the canvas: bright inside the plateau, full night
  // past the edge, the pad beyond IS the night
  const cL = LAMP_CORE * size
  const rL = LAMP_R * size
  const g = ctx.createRadialGradient(at.x, at.y, cL, at.x, at.y, rL)
  g.addColorStop(0, `rgba(0,0,0,${nightA * (1 - LAMP_E)})`)
  g.addColorStop(1, `rgba(0,0,0,${nightA})`)
  ctx.fillStyle = g
  if (hole && hole.k < 0.999) {
    ...
    cs.forEach((p, i) => (i ? ctx.lineTo(p.x, p.y) : ctx.moveTo(p.x, p.y)))
    ctx.closePath()
    ctx.clip("evenodd") // everywhere BUT the hex
    ctx.fillRect(0, 0, L.w, L.h)
    ctx.restore()
    if (hole.k > 0.001) {
      ctx.save()
      hexPath()
      ctx.clip() // …and the hex itself, at its own share of the night
      ctx.globalAlpha = hole.k
      ctx.fillRect(0, 0, L.w, L.h)
      ctx.restore()
    }
  ...
  ctx.globalAlpha = 1
  return (x, y) => {
    // the exact mirror of the painted ramp
    const t = Math.max(0, Math.min(1, (Math.hypot(x - at.x, y - at.y) - cL) / (rL - cL)))
    return 1 - nightA * (1 - LAMP_E * (1 - t))
  }
}
...
export function createRenderer(sim0) {
  ...
  let wakeBlur = false // armed at the WAKE CLICK: the ride back to the player wears motion blur
  let wakeBlurEnd = 0 // when the ride landed — the smear fades out over a short tail
  let whooshT0 = 0 // when the whoosh was armed (to disarm cleanly if no ride ever starts)
  let blurPrev = null // last frame's pixels, reused as the trail
  function draw(ctx, L, ui) {
    ...
    // THE WAKE WHOOSH (2026-08-31): while the camera rides home from wherever
    // the dream left it, each frame keeps a HEAVY print of the last with a
    // hair of outward zoom — the smear radiates, bleary, just-woken — and
    // fades out over a short tail once the ride lands.
    if (wakeBlur) {
      const gliding = camAnimating()
      if (!gliding && !blurPrev && performance.now() - whooshT0 > 250) {
        wakeBlur = false // the ride never happened (the camera was already home)
        wakeBlurEnd = 0
      } else {
        if (!gliding && !wakeBlurEnd) wakeBlurEnd = performance.now()
        const tail = wakeBlurEnd ? 1 - (performance.now() - wakeBlurEnd) / 350 : 1
        if (tail <= 0) {
          wakeBlur = false
          wakeBlurEnd = 0
          blurPrev = null
          ...
          if (blurPrev) {
            ...
            ctx.globalAlpha = 0.85 * tail
            ctx.drawImage(blurPrev, -cw * 0.006, -chh * 0.006, cw * 1.012, chh * 1.012)
            ctx.restore()
          }
          const cv = ctx.canvas
          if (!blurPrev || blurPrev.width !== cv.width || blurPrev.height !== cv.height) {
            blurPrev = document.createElement("canvas")
            blurPrev.width = cv.width
            blurPrev.height = cv.height
          }
          const bctx = blurPrev.getContext("2d")
          bctx.clearRect(0, 0, blurPrev.width, blurPrev.height)
  ...
  function drawFrame(ctx, L, ui) {
    ...
    function drawClock() {
      ...
      // the DAY LINE — the day as ONE continuous line over the dial (the lived
      // day's plateaus → the dashed way home → the dotted free time), lifting
      // off the dot-ring horizon at 00:00. OFF hides it; the 720-dot horizon
      // and the deadline pin stay either way.
      ...
      // THE HORIZON IS 720 DOTS (2026-08-30) — one per minute of the 12h face,
      // beading the orbit radius into the horizon line itself (no stroked ring:
      // the dots ARE the horizon). Every whole-minute marker — the deadline pin
      // included — lands exactly ON a dot, and the FREE-TIME stretch (below)
      // reads as this same grid lit up.
      const horizonDot = d => at((d / 720) * TAU, R) // minute-dot d's place on the ring
      ctx.fillStyle = ink
      ctx.globalAlpha = 0.25
      ctx.beginPath()
      for (let d = 0; d < 720; d++) {
        const [dx, dy] = horizonDot(d)
        ctx.moveTo(dx + 1, dy)
      ...
      if (fall > 0 && lastSegs) {
        if (SHOW_DAY_LINE) drawProfile(0, H.sleep, lastSegs.map(g => ({ ...g, h: g.h * fall })), ink)
      } else {
        if (SHOW_DAY_LINE) drawProfile(0, H.sleep, daySegs, ink)
      ...
      // to the deadline: the slack still yours. It LIGHTS UP the horizon's own
      // minute dots over its span (2026-08-31, was a dotted arc of its own —
      // the grid is the display now, the lit stretch the reading). It shrinks
      // as arrival creeps toward the deadline, and is gone once over.
      const arrive = now + cost + ret
      if (SHOW_DAY_LINE && arrive < end - 1e-9) {
        const lit = m => m >= arrive - 0.5 && m <= end + 0.5 // minute m inside the slack
        ctx.fillStyle = ink
        ctx.globalAlpha = 0.85
        ctx.beginPath()
        for (let d = 0; d < 720; d++) {
          if (!lit(d) && !lit(d + 720)) continue // each dot serves both laps of the 12h face
          const [dx, dy] = horizonDot(d)
          ctx.moveTo(dx + 1, dy)
          ctx.arc(dx, dy, 1, 0, TAU)
        }
    ...
    function drawSkills() {
      ...
      STAT_NAMES.forEach((s, i) => {
        ...
        const shR = ringR + (sunDialR - ringR) * gF
        ...
        progressPoly(mine, pf, pp, signHover && learn, fadeS, 0.35 * chromeS)
        ...
        // …a touch larger while focused — the same glyph, grown in place
        drawIcon(ctx, SKILL_ICON[s], 0, 0, iconR * (1 + 0.28 * foc), ink, (0.95 + 0.05 * (1 - menuOpen01)) * fadeS)
        ctx.restore()
        // …and the NAME reads big on the far side of the dial while focused —
        // the element's own caption, riding its focus amount (eye path: glyph →
        // shape → word, right across the dial)
        if (foc > 0.001) {
          const nnx = cx - (iconX - cx)
          const nny = cy - (iconY - cy)
          ctx.font = gameFont(Math.round(size * 1.15), 600)
          ctx.textAlign = "center"
          ctx.fillStyle = ink
          ctx.globalAlpha = 0.95 * foc
          ctx.fillText(s, nnx, nny)
          ctx.globalAlpha = 1
          ctx.textAlign = "left"
        }
        ...
        // Focused, it stays the toggle that closes the category — same element,
        // same button, its hit grown with it.
        if (ringGone <= 0.5 || (isFoc && foc > 0.5))
          skillLayout.push({ skill: s, x: iconX, y: iconY, r: isFoc ? iconR * (1 + 0.28 * foc) * 0.9 : ringR, kind: "info" })
      })
    ...
    for (const bc of Hex.range(RINGS)) {
      ...
      if (closeR && Math.hypot(cp.x - shrinkAt.x, cp.y - shrinkAt.y) < closeR * 0.87) continue // …the standalone cup draws you, above the shrinking tile
    ...
    mapLight(ctx, L, eye || lightAt, size, {
      ...
      moon,
      black: true,
      douse: !!ui.dayEnd, // asleep, the lamp is out — the dream does the seeing
      ...
      if (!ui.pending) {
        ...
        if (ui.pointer && Math.hypot(ui.pointer.x - x, ui.pointer.y - y) <= r * 0.87) {
          ...
          cursorLabel(
            ...
            L,
            ui.pointer,
            [{ text: `${Math.round(homeCharge)}m home`, color: homeCharge <= sim.energy() ? null : "#c0433a" }],
            size,
            { outward: false, stroke: ink }
          )
        }
      }
    } else if (homeSpot && !(closeR && Math.hypot(homeSpot.x - shrinkAt.x, homeSpot.y - shrinkAt.y) < closeR * 0.87)) {
    ...
    if (wakeBlur && camAnimating()) ghostCube({ x: frame.cx, y: frame.cy }, 0.6)
    ...
    onLayer("buttons", () => {
      ...
      if (ui.dayEnd) list.push({ key: "wake", icon: "wake", label: "wake", side: -1, punch: true })
    ...
    cells.forEach((c, i) => {
      ...
      else if (c.kind === "head" && head) logHead = { i: head.i, x: dx, y: CHROME_M, w, h: barM.boxH }
    ...
    daysLeft = dayCellRect ? dayCellRect.left : titleCellRect ? titleCellRect.left + titleCellRect.w : CHROME_M
  ...
  return {
    ...
    chromeHit: p =>
      [...chromeBar, logsRect, helpersRect, packRect, groundRect, profileRect, placeRect, logRunRect, logRestoreRect].some(
        r => inRect(p, r)
      ),
