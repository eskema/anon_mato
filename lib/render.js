...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    const drawEnergyCube = (x, y, r) => {
      ...
      const hue = sim.angle()
      // the sun as a LOCAL light: the unit vector from this cube to the sun's
      // actual dot on the dial. Everything — the lit faces AND the cast shadow —
      // aims along this same sun→cube line, so they agree no matter where the
      // cube stands (a cube off to the side throws its shadow outward, not just
      // "down the board").
      let tsx = sunPos.x - x
      let tsy = sunPos.y - y
      const tm = Math.hypot(tsx, tsy) || 1
      tsx /= tm // toward-sun unit x
      tsy /= tm // toward-sun unit y
