    ...
    const dots = sim.reachableDots()
    {
      ...
      for (const dlt of Hex.range(rCull)) {
        ...
        if (kind === "seam") {
          ...
          // the raw-key view (dev, "let's see how it looks"): the tile IS its
          // hex character(s) — #ccc-style grey straight from the char (the
          // centre averages its four), characters printed at the centre
          const ch = sim.nibbleAt(h)
          const avg = Math.round([...ch].reduce((s, c2) => s + parseInt(c2, 16), 0) / ch.length)
          const gray = avg.toString(16)
          fillHex(ctx, c, size, `#${gray}${gray}${gray}`, 1, o.startDeg)
