...
export function createRenderer(sim) {
  ...
  function drawPath(ctx, L, size, path, ink, surface, dashed, illegal) {
    ...
    // legible over dark ground
    ctx.strokeStyle = surface
    ctx.globalAlpha = 0.9
    ctx.lineWidth = 4.5
    ctx.stroke()
    // the trail line itself, on top — a reachable-but-unaffordable move reads in
    // a muted red instead of ink, so you see the way without it looking walkable
