...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // the sun dial — a ring around the whole board (it follows the camera's
    // board; on the seam it rings the walker), doubling as the day's CLOCK. Time
    // reads as arcs that grow along the ring from 12 o'clock, clockwise: the HOUR
    // spirals inward (30°/hour, so 6:00 is half a circle, a full day two turns),
    // the MINUTE sweeps the outer track. A solid pair is the current time; a
    // ghost pair is where the clock lands when the free budget runs out — or,
    // while hovering, right after that move. Hour + minute both step DISCRETELY
    // (whole hours / whole minutes). The dot is still the sun (day of the year).
    // Drawn as a fn so it can re-render sharp over the blurred menu.
    function drawClock() {
      const R = sunDialR
      ...
      ctx.stroke()
      // hour ticks (12; the quarters longer)
      ctx.lineCap = "butt"
      ctx.strokeStyle = ink
      for (let i = 0; i < 12; i++) {
        const a = (i / 12) * TAU
        const long = i % 3 === 0
        const inner = at(a, R - (long ? size * 0.5 : size * 0.28))
        const outer = at(a, R)
        ctx.globalAlpha = long ? 0.35 : 0.2
      ...
      // a spiral arc: sweeps `sweep` radians from 12 o'clock, its radius easing
      // rStart→rEnd across `span` radians (so each full turn draws a smaller ring)
      const spiral = (sweep, rStart, rEnd, span, width, alpha) => {
        ...
        for (let i = 0; i <= steps; i++) {
          ...
          const rr = rStart + (rEnd - rStart) * Math.min(1, t / span)
          const p = at(t, rr)
      ...
      // 06:00 + spent; discrete whole hours / whole minutes
      const hourSweep = min => (Math.floor((360 + min) / 60) / 12) * TAU // 30°/hour, spirals over 2 turns
      const minSweep = min => (Math.floor(((360 + min) % 60)) / 60) * TAU // 6°/min, one turn per hour
      const draw = (min, alpha) => {
        spiral(hourSweep(min), R * 0.7, R * 0.5, 4 * Math.PI, 3, alpha) // hour: inner, 2 turns/day
        spiral(minSweep(min), R * 0.92, R * 0.86, 2 * Math.PI, 2.5, alpha) // minute: outer track
      }
      draw(spent + cost, 0.18) // ghost (run-out / after the hovered move), under
      draw(spent, 0.6) // current time, on top
      // the sun dot (day of the year)
      ctx.globalAlpha = 1
