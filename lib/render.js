...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // the player's cube body: full surface backing (the sun shadow stays
    // strictly exterior), then the day's remaining energy as an angle-hue
    // water level draining downward, then the bold outline
    const playerCube = (x, y) => {
      ...
      ctx.fill()
      castShadow(false)
      // the day's spendable budget as liquid in the cube: full = brim, then it
      // drains vertically. The surface is a 3D diamond (the cube's top face) so
      // the level sinks as a diamond, not a flat line
      const frac = Math.max(0, Math.min(1, freeEnergy / ENERGY_START))
      if (frac > 0) {
        const half = 0.8660254 * r // flat-top hex reaches ±0.866r in y
        const yTop = y - half
        const yBot = y + half
        const yc = yBot - frac * (yBot - yTop) // the waterline's centre
        const hw = r - 0.57735 * Math.min(half, Math.abs(yc - y)) // silhouette half-width there
        const d = 0.32 * r // the surface diamond's half-thickness
        const hue = sim.angle()
        ctx.save()
        path()
        ctx.clip()
        // the body below the surface (its near edge dips to a front apex)
        ctx.beginPath()
        ctx.moveTo(x - r, yBot)
        ctx.lineTo(x - r, yc)
        ctx.lineTo(x - hw, yc)
        ...
        ctx.globalAlpha = 0.9
        ctx.fill()
        // the top face of the liquid — a lighter diamond
        ctx.beginPath()
        ctx.moveTo(x, yc + d)
        ctx.lineTo(x - hw, yc)
        ctx.lineTo(x, yc - d)
    ...
    const drawStack = (x, y, hasPlayer, hasNpc) => {
      ...
      if (hasNpc) {
        ...
        boldY(x, y, size / 3) // the player's identity
      } else {
    ...
    // the player anywhere else: the standalone cube — bold hex + bold Y
    if (!playerDrawn) {
      const pc = hexToPixel(L, cubeTile[0], cubeTile[1], size)
      playerCube(pc.x, pc.y)
      boldY(pc.x, pc.y, size / 3)
    }
