...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // EVERY board's centre is a special tile — the resting place of its
    // figure — drawn like home's centre: a hexagon with all six radial lines.
    const centreMark = (cx, cy) => {
      const cs = []
      for (let i = 0; i < 6; i++) {
        const a = (Math.PI / 180) * (60 * i + o.startDeg)
        cs.push([cx + size * Math.cos(a), cy + size * Math.sin(a)])
      }
      ctx.strokeStyle = ink
      ctx.globalAlpha = 0.12
      ctx.lineWidth = 1
    ...
    const centres = [] // { bc, centre, cp, npc } for discovered board centres in view
    for (const bc of Hex.range(RINGS)) {
      ...
      centreMark(cp.x, cp.y)
      centres.push({ bc, centre, cp, npc: sim.npcAt(bc) })
    }
    // the player cube — offset DOWN when sharing a figure's centre, so both
    // stack and stay visible (the figure sits above, drawn on top after)
    const onFigure = centres.some(c => c.npc && eq(c.centre, cubeTile))
    const pc = hexToPixel(L, cubeTile[0], cubeTile[1], size)
    castShadow(true)
    drawCube(ctx, pc.x, pc.y + (onFigure ? size * 0.3 : 0), size, ink, surface, o.startDeg)
    castShadow(false)
    // the resting figures on top, offset UP when the player shares their tile
    for (const c of centres) {
      if (!c.npc) continue
      const up = eq(c.centre, cubeTile) ? size * 0.32 : 0
      drawCube(ctx, c.cp.x, c.cp.y - up, size * 0.72, ink, surface, o.startDeg, false, 0.85)
    }

    // the radial menu — laid out on the REAL hex grid (6 slots on the
    // player's neighbours; a folder fans onto outward cells, ≤3 then ≤5).
    // When open the world behind blurs + dims and the current tile is punched
    // back out sharp (player + figure stacked, visible through the hole).
    menuLayout = null
    if (ui.menu) {
      const P = v.player
      const hub = hexToPixel(L, P[0], P[1], size)

      // blur + dim the whole world, then restore the current tile sharp
      const cv = ctx.canvas
      ...
      ctx.globalAlpha = 1
      ctx.save() // punch: clip to the current tile, redraw it sharp from the snapshot
      const punch = hexCorners(hub.x, hub.y, size * 1.2, o.startDeg)
      ctx.beginPath()
      ...
      if (them.length) {
        // the ring splits: YOU on the 3 left cells, the thing you inspect
        // (land or figure) on the 3 right cells — both stand on this tile
        const px = cell => hexToPixel(L, cell[0], cell[1], size).x
        const py = cell => hexToPixel(L, cell[0], cell[1], size).y
        const byX = ring1.slice().sort((a, b) => px(a) - px(b))
        const leftCells = byX.slice(0, 3).sort((a, b) => py(a) - py(b))
        const rightCells = byX.slice(3).sort((a, b) => py(a) - py(b))
        self.forEach((n, i) => leftCells[i] && cells.push({ node: n, cell: leftCells[i] }))
        them.forEach((n, i) => rightCells[i] && cells.push({ node: n, cell: rightCells[i] }))
      } else {
        self.forEach((n, i) => ring1[i] && cells.push({ node: n, cell: ring1[i] }))
      }
      for (const c of cells) used.add(key(c.cell))

      // the open folder fans its children onto outward cells: the (≤3) ring-2
      // cells beyond its slot, then (≤5) ring-3 cells past those
      const open = cells.find(c => c.node.id === ui.menu.openId)
      if (open && open.node.children?.length) {
        const near = neigh0 => Hex.neighbors(neigh0)
        const r2 = near(open.cell)
          .filter(c => Hex.distance(P, c) === 2 && !used.has(key(c)))
          .sort((a, b) => angDiff(ang(a), ang(open.cell)) - angDiff(ang(b), ang(open.cell)))
          .slice(0, 3)
        r2.forEach(c => used.add(key(c)))
        const r3 = []
        for (const c of r2)
          for (const n of Hex.neighbors(c))
            if (Hex.distance(P, n) === 3 && !used.has(key(n)) && !r3.some(x => eq(x, n))) {
              r3.push(n)
              used.add(key(n))
            }
        r3.sort((a, b) => angDiff(ang(a), ang(open.cell)) - angDiff(ang(b), ang(open.cell)))
        const outCells = [...r2, ...r3.slice(0, 5)]
        open.node.children.forEach((n, i) => outCells[i] && cells.push({ node: n, cell: outCells[i], child: true }))
      }

      // background: a padded silhouette — occupied cells as oversized hexes in
      // ONE path (no seams), ~quarter-tile padding. The current tile is left
      // OUT so the punched-out sharp tile shows through the hole.
      ctx.save()
      ctx.fillStyle = surface
      ctx.globalAlpha = 0.92
      ctx.beginPath()
      for (const cellKey of used) {
        if (cellKey === key(P)) continue // the punched-out hole
        const [cq, cr] = cellKey.split(",").map(Number)
        const cp2 = hexToPixel(L, cq, cr, size)
      ...
      for (const c of cells) {
        ...
        const A = faded ? 0.3 : 1
        const cs = hexCorners(p.x, p.y, size * 0.9, o.startDeg)
        ctx.beginPath()
        ...
        ctx.fill()
        ctx.strokeStyle = ink
        ctx.globalAlpha = (focus ? 1 : on ? 0.55 : 0.25) * A
        ctx.lineWidth = focus ? 2.5 : 1.5
        ctx.stroke()
        ctx.globalAlpha = 1
