...
export function createRenderer(sim) {
  ...
  // ── draw pieces ────────────────────────────────────
  function drawPath(ctx, L, size, path, ink, surface, dashed, illegal) {
    ...
    // a surface-coloured halo underneath, 3× the line width, so the trail stays
    // legible over dark ground — ALWAYS solid, even when the line dashes, so the
    // border reads as one continuous outline
    ctx.setLineDash([])
    ctx.strokeStyle = surface
    ctx.globalAlpha = 0.9
    ctx.lineWidth = 4.5
    ctx.stroke()
    // the trail line itself, on top — dashed only if asked. A reachable-but-
    // unaffordable move reads in a muted red instead of ink, so you see the way
    // without it looking walkable
    if (dashed) ctx.setLineDash([5, 5])
    ctx.strokeStyle = illegal ? "#c0433a" : ink
    ctx.globalAlpha = illegal ? 0.7 : 0.45
    ctx.lineWidth = 1.5
    ctx.stroke()
    ctx.restore()
  }

  // A solid arrowhead (width 4) with a surface-coloured border under it, so it
  // stays legible over dark ground — the arrow twin of drawPath's halo.
  function arrowHalo(ctx, fromX, fromY, toX, toY, color, surface, size) {
    const len = size * 0.32
    const halfW = size * 0.2
    ctx.globalAlpha = 0.9
    arrowTip(ctx, fromX, fromY, toX, toY, surface, len, halfW, 7) // the border — wide, so white shows on each side
    ctx.globalAlpha = 0.6
    arrowTip(ctx, fromX, fromY, toX, toY, color, len, halfW, 1.5) // the arrow itself — thin
    ctx.globalAlpha = 1
  }

  // Arrowheads along the committed trail — one per leg, at each segment midpoint,
  // pointing in the direction travelled. A leap leg is just a longer segment:
  // same straight arrow, riding the crack between the flankers.
  function drawTrailArrows(ctx, L, size, trail, ink, surface) {
    if (trail.length < 2) return
    for (let i = 1; i < trail.length; i++) {
      const a = hexToPixel(L, trail[i - 1][0], trail[i - 1][1], size)
      const b = hexToPixel(L, trail[i][0], trail[i][1], size)
      arrowHalo(ctx, a.x, a.y, (a.x + b.x) / 2, (a.y + b.y) / 2, ink, surface, size)
    }
  }
  ...
  function draw(ctx, L, ui) {
    ...
    drawPath(ctx, L, size, trail, ink, surface, false)
    drawTrailArrows(ctx, L, size, trail, ink, surface)
    drawPath(ctx, L, size, ui.hoverPath, ink, surface, true, ui.hoverIllegal)
    if (ui.hoverPath && ui.hoverPath.length >= 2) {
      ...
      arrowHalo(ctx, a.x, a.y, b.x, b.y, ui.hoverIllegal ? "#c0433a" : ink, surface, size)
    }
