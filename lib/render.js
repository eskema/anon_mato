...
export function createRenderer(sim) {
  ...
  // is [{ text, color?, alpha? }].
  function cursorLabel(ctx, L, pointer, lines) {
    ...
    // the cursor sits at the same fraction ALONG the box as it does ACROSS the
    // viewport: near the left edge the box is fully to its right, near the right
    // edge fully to its left, sliding smoothly between — always leaning toward
    // centre, with no corner jumps at any threshold.
    const fx = pointer.x / L.w
    const fy = pointer.y / L.h
    let left = pointer.x - boxW * fx
    let top = pointer.y - boxH * fy
  ...
  function draw(ctx, L, ui) {
    ...
    if (ui.menu) {
      ...
      if (f && f.node.label) cursorLabel(ctx, L, ui.pointer, [{ text: f.node.label }])
      menuLayout = { hits }
    }
