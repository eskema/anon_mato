...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    if (ui.menu) {
      ...
      const baseCells = [P, ...cells.filter(c => !c.child).map(c => c.cell)]
      ctx.save()
      ctx.fillStyle = surface
      ctx.globalAlpha = 0.92
      silhouette(baseCells)
      ctx.fill()
      if (folderCells) {
        // the darker rim: the same union nudged out ~1.5px, filled in ink,
        // then the surface group on top — reads as a thin border
        ctx.globalAlpha = 0.4
        ctx.fillStyle = ink
        ctx.beginPath()
        for (const cell of folderCells) {
          const cp2 = hexToPixel(L, cell[0], cell[1], size)
          const cs = hexCorners(cp2.x, cp2.y, size * 1.25 + 1.5, o.startDeg)
          cs.forEach((pt, k2) => (k2 ? ctx.lineTo(pt.x, pt.y) : ctx.moveTo(pt.x, pt.y)))
          ctx.closePath()
        }
        ctx.fill()
      ...
      // the punched-out tile, ABOVE the menu: clip to the current tile and
      // redraw it sharp from the snapshot — ground, then player, then figure,
      // exactly the stacking the world pass drew
      ctx.save()
      const punch = hexCorners(hub.x, hub.y, size * 1.15, o.startDeg)
      ctx.beginPath()
