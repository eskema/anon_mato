...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // pass 1 — the surface silhouette, under everything (always solid)
    strokePath(ctx, L, size, trail, surface, 4.5, 0.9)
    strokePath(ctx, L, size, hov, surface, 4.5, 0.9)
    trailArrows(ctx, L, size, trail, surface, 7, 0.9)
    if (tb) {
      if (bad) crossTip(ctx, tb.x, tb.y, surface, size, 6, 0.9)
      else {
        ctx.globalAlpha = 0.9
    ...
    // pass 2 — the ink lines + heads on top
    strokePath(ctx, L, size, trail, ink, 1.5, 0.45)
    strokePath(ctx, L, size, hov, tipInk, 1.5, bad ? 0.7 : 0.45, [5, 5])
    trailArrows(ctx, L, size, trail, ink, 1.5, 0.6)
    if (tb) {
      if (bad) crossTip(ctx, tb.x, tb.y, tipInk, size, 2, 0.8)
      else {
        ctx.globalAlpha = 0.6
