...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // committed walked route (solid, per-leg arrowheads) + dashed hover preview,
    // drawn as ONE surface silhouette first, then the ink lines/heads on top —
    // so the border sits beneath the whole trail, not over it. An illegal hover
    // ends in an X (in muted red) instead of an arrowhead.
    const hov = ui.hoverPath
    const bad = ui.hoverIllegal
    const RED = "#c0433a"
    // legal: surface border + ink line. ILLEGAL inverts it — a RED border with a
    // surface-coloured dashed line (and X) riding on top, so it clearly reads as
    // forbidden while staying legible on any ground.
    const hovBorder = bad ? RED : surface
    const hovLine = bad ? surface : ink
    let ta = null
    let tb = null
    if (hov && hov.length >= 2) {
    ...
    // pass 1 — the border silhouette, under everything (always solid)
    strokePath(ctx, L, size, trail, surface, 4.5, 0.9)
    strokePath(ctx, L, size, hov, hovBorder, 4.5, 0.9)
    trailArrows(ctx, L, size, trail, surface, 7, 0.9)
    if (tb) {
      if (bad) crossTip(ctx, tb.x, tb.y, RED, size, 6, 0.9)
      else {
        ctx.globalAlpha = 0.9
        arrowTip(ctx, ta.x, ta.y, tb.x, tb.y, surface, size * 0.32, size * 0.2, 7)
        ctx.globalAlpha = 1
      }
    }
    // pass 2 — the lines + heads on top
    strokePath(ctx, L, size, trail, ink, 1.5, 0.45)
    strokePath(ctx, L, size, hov, hovLine, 1.5, bad ? 0.95 : 0.45, [5, 5])
    trailArrows(ctx, L, size, trail, ink, 1.5, 0.6)
    if (tb) {
      if (bad) crossTip(ctx, tb.x, tb.y, surface, size, 2, 0.95)
      else {
        ctx.globalAlpha = 0.6
