...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    let tb = null
    if (hov && hov.length >= 2) {
      ta = hexToPixel(L, hov[hov.length - 2][0], hov[hov.length - 2][1], size)
      tb = hexToPixel(L, hov[hov.length - 1][0], hov[hov.length - 1][1], size)
    }
    strokePath(ctx, L, size, trail, ink, 1.5, 0.6)
    trailArrows(ctx, L, size, trail, ink, 1.5, 0.6)
    strokePath(ctx, L, size, hov, hovColor, 1.5, bad ? 0.7 : 0.6, [5, 5])
    if (tb) {
      if (bad) crossTip(ctx, tb.x, tb.y, hovColor, size, 2, 0.8)
      else {
        ctx.globalAlpha = 0.6
        arrowTip(ctx, ta.x, ta.y, tb.x, tb.y, ink, size * 0.32, size * 0.2, 1.5)
        ctx.globalAlpha = 1
      }
    }
