...
export function createRenderer(sim) {
  ...
  // The SAME font must measure (layout) and draw (box), or the box mis-sizes.
  ...
  function labelLayout(ctx, L, pointer, lines, tile, outward = false, keepClear = null) {
    ...
    const pad = 6
    const lineH = 22
    const widths = lines.map(l => ctx.measureText(l.text).width + pad * 2)
    const boxW = Math.max(...widths)
    const boxH = lineH * lines.length
    // which way the box sits off the cursor:
    let dx, dy
    if (keepClear && !outward) {
      // a THIRD direction — PERPENDICULAR to the player→cursor line, so the
      // player, cursor and label form a triangle and the box never sits on the
      // cube (toward-centre would aim straight at the player). Take the side
      // heading toward the viewport centre, so it stays on-screen.
      let px = pointer.x - keepClear.x
      let py = pointer.y - keepClear.y
      const pl = Math.hypot(px, py) || 1
      px /= pl
      py /= pl
      const towardC = -py * (L.cx - pointer.x) + px * (L.cy - pointer.y) // perp·(centre−cursor)
      dx = towardC >= 0 ? -py : py
      dy = towardC >= 0 ? px : -px
    } else {
      // toward the viewport centre (or AWAY when `outward`, e.g. the menu label)
      const s = outward ? -1 : 1
      dx = (L.cx - pointer.x) * s
      dy = (L.cy - pointer.y) * s
    ...
    const reach = gap + (Math.abs(dx) * boxW + Math.abs(dy) * boxH) / 2
    let left = pointer.x + dx * reach - boxW / 2
    let top = pointer.y + dy * reach - boxH / 2
    const clampIn = () => {
      left = Math.max(4, Math.min(L.w - boxW - 4, left))
      top = Math.max(4, Math.min(L.h - boxH - 4, top))
    }
    clampIn()
    // never overlap the player: if the box intrudes on its clearance circle, push
    // the box straight out of it (a few passes so re-clamping settles)
    if (keepClear) {
      for (let i = 0; i < 5; i++) {
        const nx = Math.max(left, Math.min(keepClear.x, left + boxW))
        const ny = Math.max(top, Math.min(keepClear.y, top + boxH))
        let ox = nx - keepClear.x
        let oy = ny - keepClear.y
        let d = Math.hypot(ox, oy)
        if (d >= keepClear.r) break // already clear
        if (d < 0.001) {
          // player at the nearest point (inside the box) — push toward its centre
          ox = left + boxW / 2 - keepClear.x
          oy = top + boxH / 2 - keepClear.y
          d = Math.hypot(ox, oy) || 1
        }
        left += (ox / d) * (keepClear.r - d)
        top += (oy / d) * (keepClear.r - d)
        clampIn()
      }
    }
    return { lines, left, top, lineH, pad, widths, pointer }
  }
  ...
  function labelFan(ctx, lay, stroke = "#fff") {
    if (!lay) return
    const { lines, left, top, lineH, widths, pointer } = lay
    ...
    lines.forEach((l, i) => {
      const y = top + i * lineH
      for (const [a, b] of boxFrontEdges(pointer, left, y, widths[i], lineH)) {
        for (const c of [a, b]) {
          const k = c[0] + "," + c[1]
  ...
  function labelBox(ctx, lay, border = "#fff") {
    if (!lay) return
    const { lines, left, top, lineH, pad, widths } = lay
    ctx.font = LABEL_FONT // must match labelLayout's measurement, or the box over-sizes
    ctx.textAlign = "left"
  ...
  function cursorLabel(ctx, L, pointer, lines, tile, { outward = false, stroke = "#fff", keepClear = null } = {}) {
  ...
  // ── the frame ──────────────────────────────────────
  // ui: { hovered, hoverPath, hoverRetrace, hoverIllegal, pointer, logsOpen, replaying, menu, statsPanel,
  ...
  function draw(ctx, L, ui) {
    ...
    // the sun's height across the day: overhead at noon (mid-budget), low at the
    // ends. Shadows vanish at noon and stretch longest at sunrise/sunset.
    const dayExtreme = Math.abs(1 - 2 * (spent / ENERGY_START)) // 0 at noon → 1 at the ends
    const sunLen = 14 * dayExtreme // walls' cast-shadow offset (px)
    // the sun's ACTUAL screen position on the dial — shadows are cast FROM this
    // point (a local light: the direction away from it differs per cube, running
    // along the sun→cube line), and the dial dot is drawn here too
    const sunCentre = hexToPixel(L, camAnchor[0], camAnchor[1], size)
    const sunDialR = size * Math.sqrt(3) * (SEAM_RING + 0.7)
    const sunPos = { x: sunCentre.x + Math.sin(sunRad) * sunDialR, y: sunCentre.y - Math.cos(sunRad) * sunDialR }
    // ONE shadow/light direction for the whole board, taken from the CENTRE tile
    // as the template: the unit vector from the board centre to the sun's dot.
    // Every cube uses this same direction (no per-move recompute) so the light
    // reads as one distant source, not a spotlight following each figure.
    const sunTo = (() => {
      const dx = sunPos.x - sunCentre.x
      const dy = sunPos.y - sunCentre.y
    ...
    // the sun dial — the day's CLOCK, ringing the board. Everything sits INSIDE
    // the orbit ring, along the hour marks. Time is ONE continuous line (the hour,
    // fine so it advances by the minute; 30°/hour from 12 o'clock, 6:00 = half a
    // circle). Its first 12h fill the band between the BIG and small mark tips
    // (thickness = half a big mark, so it's flush with both); past 11:59 it steps
    // out into the small-mark slot for the next 12h. A thin brighter ghost runs on
    // to where the free budget (or the hovered move) lands; a bold radial tick
    // marks where the day ENDS (07:00). All caps square. Dot = sun (day of year).
    function drawClock() {
      const R = sunDialR
      const cx = sunCentre.x
      const cy = sunCentre.y
      const TAU = Math.PI * 2
      const big = size * 0.4 // long (quarter) hour-mark length, from the ring inward
      const small = big / 2 // short hour-mark length — half the big
      const bandW = big / 2 // spiral band thickness = the gap between the two mark tips
      const c0 = R - big + bandW / 2 // turn-0 band centre (between big & small tips)
      const c1 = R - small + bandW / 2 // turn-1 band centre (the small-mark slot)
      const at = (a, rad) => [cx + Math.sin(a) * rad, cy - Math.cos(a) * rad] // 0 = 12 o'clock, clockwise
      // the sun's orbit — opaque
      ctx.strokeStyle = ink
      ctx.globalAlpha = 1
      ctx.lineWidth = 1.5
      ctx.beginPath()
      ...
      ctx.stroke()
      // 12 hour marks, inward from the ring (full alpha; quarters longer)
      ctx.lineCap = "butt"
      for (let i = 0; i < 12; i++) {
        const a = (i / 12) * TAU
        const long = i % 3 === 0
        const p0 = at(a, R - (long ? big : small))
        const p1 = at(a, R)
      ...
      // radius vs. angle: a flat circle through most of a 12h turn, curving out to
      // the next turn's band only in its last stretch (turn 0 → turn 1)
      const radiusAt = t => {
        const tf = t / TAU // turns elapsed (0..2 over 24h)
        const turn = Math.min(1, Math.floor(tf))
        const within = tf - turn
        const rIn = turn === 0 ? c0 : c1
        const k = within < 0.85 ? 0 : (within - 0.85) / 0.15
        return rIn + (c1 - rIn) * k
      }
      // one continuous line from 12 o'clock to `mins`, advancing by the minute
      const line = (mins, width, alpha) => {
        const sweep = (Math.floor(mins) / 720) * TAU // 720 min = 12h = one turn
        ...
        ctx.lineJoin = "miter"
        ctx.beginPath()
        const steps = Math.max(2, Math.ceil(sweep / 0.04))
        for (let i = 0; i <= steps; i++) {
          const t = sweep * (i / steps)
          const p = at(t, radiusAt(t))
          i ? ctx.lineTo(p[0], p[1]) : ctx.moveTo(p[0], p[1])
        }
        ctx.stroke()
      }
      // hovered action's cost (else the whole free budget): where the clock lands
      let cost = freeEnergy
      if (!ui.pending && ui.hovered) {
        if (ui.hoverPath) cost = sim.pathCost(ui.hoverPath)
        else if (sim.isFrontier(ui.hovered) && sim.canScout(ui.hovered)) cost = sim.scoutCostAt(ui.hovered)
      }
      const now = 360 + spent // minutes since midnight (06:00 + spent)
      line(now + cost, 3, 0.45) // ghost: on to the run-out / after the hovered move — thin, brighter
      line(now, bandW, 0.85) // current time — the thick band, flush between the mark tips
      // where the day ENDS (07:00 for now = 06:00 + the full budget): a bold radial
      // tick from the big-mark depth out to the ring, inside — the finish line
      const ea = ((360 + ENERGY_START) / 720) * TAU
      const ei = at(ea, R - big)
      const eo = at(ea, R)
      ctx.strokeStyle = ink
      ctx.globalAlpha = 1
      ctx.lineWidth = 3.5
      ctx.beginPath()
      ctx.moveTo(ei[0], ei[1])
      ctx.lineTo(eo[0], eo[1])
      ctx.stroke()
      // the sun dot (day of the year), on the orbit
      ctx.globalAlpha = 1
    ...
    if (ui.pointer && !ui.menu && !ui.pending && ui.hovered) {
      ...
      const l2 = hb
        ? `[${h[0] - sim.boardCentreOf(h)[0]},${h[1] - sim.boardCentreOf(h)[1]}]`
        : `[${h[0]},${h[1]}]`
      let l1 = null
      if (ui.hoverPath) {
        const ct = ui.pending?.ghostTile || v.player
        const cb = sim.boardHexOf(ct)
        const crossing = !!hb && (!cb || hb[0] !== cb[0] || hb[1] !== cb[1])
        l1 = `+${Math.round(sim.pathCost(ui.hoverPath))} ${crossing ? "crossing to" : "walking to"}`
      } else if (sim.isFrontier(h) && sim.canScout(h)) {
        l1 = `+${Math.round(sim.scoutCostAt(h))} scouting`
      }
      if (l1)
        hoverLabel = labelLayout(
          ctx,
          L,
          ui.pointer,
          [
            { text: l1, color: ui.hoverIllegal ? "#c0433a" : "#111" },
            { text: l2, alpha: 0.6 }
          ],
          size,
          false,
    ...
    strokePath(ctx, L, size, trail, ink, 1.5, 0.6)
    trailArrows(ctx, L, size, trail, ink, 1.5, 0.6)
    ...
    const drawEnergyCube = (x, y, r) => {
      ...
      const hue = sim.angle()
      // one board-wide sun direction (computed from the centre tile). Both the
      // lit faces (toward the sun) and the cast shadow (away) aim along it.
      const [tsx, tsy] = sunTo // toward-sun unit
      // sun-lit faces: each of the cube's three faces brightens as it turns to
      // face the sun and darkens on the shadow side. Face screen-normals are up
      // (lid), down-right, down-left.
      const lit = (nx, ny, baseL, swing) => {
        const d = nx * tsx + ny * tsy // ·(toward-sun)
      ...
      ctx.globalAlpha = 1
      ctx.fill()
      // 3) the liquid, clipped to the cube
      ...
      ctx.lineJoin = "round"
      ctx.stroke()
      // 5) the identity Y — two arms up to the side faces, a stem down the front edge
      const ell = r / 2
      ctx.strokeStyle = ink
      ctx.lineWidth = W
      ctx.lineCap = "round"
      ...
      ctx.lineTo(x, y + ell)
      ctx.stroke()
      // 6) the budget readout — the day's minutes left, on the TOP FACE where the
      // pupil sat. While hovering a move it previews what you'd have LEFT after
      // paying for it, in a lighter shade (the top status line is unchanged).
      let mins = Math.round(freeEnergy)
      let faded = false
      if (!ui.pending && ui.hovered && ui.hoverPath) {
        mins = Math.round(freeEnergy - sim.pathCost(ui.hoverPath))
        faded = true
      } else if (!ui.pending && ui.hovered && sim.isFrontier(ui.hovered) && sim.canScout(ui.hovered)) {
        mins = Math.round(freeEnergy - sim.scoutCostAt(ui.hovered))
        faded = true
      }
      ctx.fillStyle = ink
      ctx.globalAlpha = faded ? 0.4 : 0.95
      ctx.font = `700 ${Math.round(r * 0.36)}px system-ui, sans-serif`
      ctx.textAlign = "center"
      ctx.textBaseline = "middle"
      ctx.fillText(`${mins}`, x, y - r / 2)
      ctx.globalAlpha = 1
    }
    ...
    if (ui.menu) {
      ...
      // the focused node's label — rendered in the same cursor space as the
      // tile-hover readout, so every hover reads consistently
      const f = cells.find(c => c.node.id === ui.menu.focusId)
      if (f && f.node.label)
        cursorLabel(ctx, L, ui.pointer, [{ text: f.node.label }], size, { outward: true, stroke: "#111", keepClear: playerClear() })
      menuLayout = { hits }
    }
    ...
    // the header keeps the real one, so no dot is passed for it). Ink centre with
    // a surface ring, so it separates from any ground on either theme.
    if (ui.pointer) {
      ctx.globalAlpha = 1
      ctx.beginPath()
