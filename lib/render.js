...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // per item UNIT, its initial letter as its face (all eight item names start
    // uniquely: fish plants wood rock eggs metal basket axe). The in-use tool
    ...
    function drawSideViews() {
      const chipR = Math.max(11, size * 0.5)
      const chip = (x, y, letter, { dashed = false, alarm = false, uses = null } = {}) => {
        ctx.beginPath()
        for (let k = 0; k < 6; k++) {
          const a = (Math.PI / 180) * (30 + 60 * k) // pointy — an item is tile-scale
          const px = x + chipR * Math.cos(a)
          const py = y + chipR * Math.sin(a)
          k ? ctx.lineTo(px, py) : ctx.moveTo(px, py)
        }
        ctx.closePath()
        ctx.fillStyle = surface
        ctx.globalAlpha = dashed ? 0.7 : 0.96
        ctx.fill()
        ctx.strokeStyle = alarm ? "#c0433a" : ink
        ctx.globalAlpha = dashed ? 0.7 : 0.9
        ctx.lineWidth = 1.5
        ctx.setLineDash(dashed ? [3, 3] : [])
        ctx.stroke()
        ctx.setLineDash([])
        ctx.fillStyle = alarm ? "#c0433a" : ink
        ctx.globalAlpha = 1
        ...
        if (uses != null) {
          ...
          ctx.globalAlpha = 0.6
          ctx.fillText(String(uses), x + chipR * 0.9, y + chipR * 0.75)
          ctx.globalAlpha = 1
        }
      }
      const pitch = chipR * 1.95
      const rowY = L.h - 14 - chipR
      let x = 14 + chipR
      for (const d of sim.packDetail())
        for (let u = 0; u < d.n; u++) {
          chip(x, rowY, d.k[0], {
            alarm: d.spoilsIn != null && d.spoilsIn <= 60,
            uses: u === 0 && d.uses != null ? d.uses : null
          })
          x += pitch
        }
      ctx.font = "600 11px system-ui, sans-serif"
      ...
      ctx.globalAlpha = 0.7
      ctx.fillText(`load ${Math.round(sim.loadOf() * 10) / 10}/${sim.carryCap()}`, 14, rowY - chipR - 10)
      ctx.globalAlpha = 1
      const st = sim.stashHere()
      if (st) {
        // the buried row — dashed chips, in the ground beneath your feet
        let sx = 14 + chipR
        const sy = rowY - chipR - 24 - chipR
        const shown = Math.min(st.n, 10)
        for (let u = 0; u < shown; u++) {
          chip(sx, sy, st.item[0], { dashed: true })
          sx += pitch
        }
        if (st.n > shown) {
          ctx.font = "600 11px system-ui, sans-serif"
          ctx.fillStyle = nightInk
          ctx.globalAlpha = 0.7
          ctx.fillText(`+${st.n - shown}`, sx - chipR + 4, sy)
          ctx.globalAlpha = 1
        }
      }
    ...
    const paintField = (ctx, pad = 0) => {
      ...
      for (const dlt of Hex.range(rCull)) {
        ...
        if (bits) {
          ...
          ctx.globalAlpha = 0.7
          ctx.lineWidth = 3
          // PROJECTING terminals: each wall is stroked as its own segment, so a
          // butt cap cut it dead at the corner and left a notch where two met —
          // the run read as broken pieces. A square cap carries each end half a
          // width PAST the vertex along its own line, so neighbours overlap into
          // the corner wedge and the run reads continuous (and a lone wall gets a
          // squared-off terminal instead of a hairline cut).
          ctx.lineCap = "square"
          castShadow(ctx, true)
          for (let d = 0; d < 6; d++) {
