...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // the player cube — and the figures NESTED above it, all concentric, in
    // THIRDS: tile (R) ⊃ player (2R/3) ⊃ figure (R/3), so the gap from tile
    // to player equals the gap from player to figure. drawCube takes the
    // "tile width", whose corner radius is half of it — hence ×4/3 and ×2/3.
    const pc = hexToPixel(L, cubeTile[0], cubeTile[1], size)
    castShadow(true)
    drawCube(ctx, pc.x, pc.y, size * (4 / 3), ink, surface, o.startDeg)
    castShadow(false)
    // the resting figures on top — a third of the tile, dead centre
    for (const c of centres) {
      if (!c.npc) continue
      drawCube(ctx, c.cp.x, c.cp.y, size * (2 / 3), ink, surface, o.startDeg)
    }
