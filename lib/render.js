...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    const sunLen = 6 + 11 * Math.min(1, Math.abs(6 + spent / 60 - 14) / 8)
    ...
    const drawEnergyCube = (x, y, r) => {
      ...
      // 4) the eye — a coloured eyeball (iris + pupil + glint) that slides as ONE
      // toward the pointer within the TOP FACE. Its socket is the lid DIAMOND and
      // the iris is as tall as it, so the diamond cuts the eyeball as it rolls;
      // drawn UNDER both the outline AND the Y creases (steps 5–6), so every edge
      // caps it and it tucks into the face. Recentres when the pointer's on the cube.
      const dCx = x
      const dCy = y - r / 2 // the lid's centre
      const irisR = r * 0.5 // as tall as the diamond (peak → centre)
      const glintDist = r * 0.15 // the glint's distance from the pupil centre (unchanged)
      const pupilR = glintDist // pupil shrunk so its rim crosses the glint's middle
      const glintR = r * 0.09
      const maxTravel = r * 0.22
      let ox = 0
      let oy = 0
      if (ui.pointer && Math.hypot(ui.pointer.x - x, ui.pointer.y - y) >= r) {
        const dx = ui.pointer.x - dCx
        const dy = ui.pointer.y - dCy
        const m = Math.hypot(dx, dy) || 1
      ...
      ctx.fillStyle = ink
      ctx.fill()
      // the glint — a white reflection on the pupil at the CHOSEN ANGLE's spot on
      // the wheel (0°=up, clockwise): the pupil is the sun's orbit, this is the
      // sun frozen at the player's angle, so the eye reflects the angle choice
      const aRad = (hue * Math.PI) / 180
      ctx.beginPath()
      ctx.arc(dCx + ox + Math.sin(aRad) * glintDist, dCy + oy - Math.cos(aRad) * glintDist, glintR, 0, Math.PI * 2)
      ctx.fillStyle = "#fff"
      ctx.fill()
      ctx.restore()
      // 5) the bold outline — on top, so it caps the eye at the OUTER edges
      hex()
      ctx.strokeStyle = ink
      ctx.lineWidth = W
      ctx.lineJoin = "round"
      ctx.stroke()
      // 6) the identity Y — also on top, so the INTERIOR creases (its two upper
      // arms) cap the eye's lower edges the same way the border caps its upper
      // ones; the stem runs down the front
      const ell = r / 2
      ctx.strokeStyle = ink
      ctx.lineWidth = W
      ctx.lineCap = "round"
