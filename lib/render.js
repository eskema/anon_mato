...
export function createRenderer(sim) {
  ...
  // ── draw pieces ────────────────────────────────────
  // A leap segment (2 tiles out) draws as a lifted arc over the jumped tile —
  // this is its control point, a perpendicular bulge off the midpoint.
  function leapControl(a, b, size) {
    const dx = b.x - a.x
    const dy = b.y - a.y
    const len = Math.hypot(dx, dy) || 1
    const lift = size * 0.9 // peak sits ~0.45 hex off the line
    return { x: (a.x + b.x) / 2 - (dy / len) * lift, y: (a.y + b.y) / 2 + (dx / len) * lift }
  }
  ...
  function drawPath(ctx, L, size, path, ink, dashed) {
    ...
    ctx.beginPath()
    let a = hexToPixel(L, path[0][0], path[0][1], size)
    ctx.moveTo(a.x, a.y)
    for (let i = 1; i < path.length; i++) {
      const b = hexToPixel(L, path[i][0], path[i][1], size)
      if (Hex.distance(path[i - 1], path[i]) > 1) {
        const c = leapControl(a, b, size) // a leap — hop the jumped-over tile
  ...
  // midpoint, pointing in the direction travelled. Leap legs put the arrow at
  // the arc's peak, riding the hop.
  function drawTrailArrows(ctx, L, size, trail, ink) {
    ...
    for (let i = 1; i < trail.length; i++) {
      ...
      if (Hex.distance(trail[i - 1], trail[i]) > 1) {
        const c = leapControl(a, b, size)
        const peak = { x: (a.x + b.x) / 4 + c.x / 2, y: (a.y + b.y) / 4 + c.y / 2 } // the curve at t=0.5
        arrowTip(ctx, a.x, a.y, peak.x, peak.y, ink, size * 0.32, size * 0.2, 1.5)
      } else {
  ...
  // ── the frame ──────────────────────────────────────
  // ui: { hovered, hoverPath, hoverRetrace, clockExpanded, replaying, menu,
