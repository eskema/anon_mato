...
export function createRenderer(sim) {
  ...
  function labelLayout(ctx, L, pointer, lines, tile, outward = false) {
  ...
  function cursorLabel(ctx, L, pointer, lines, tile, { outward = false, stroke = "#fff" } = {}) {
    const lay = labelLayout(ctx, L, pointer, lines, tile, outward)
  ...
  function draw(ctx, L, ui) {
    ...
    if (ui.pointer && !ui.menu && !ui.pending && ui.hovered) {
      ...
      if (l1)
        hoverLabel = labelLayout(ctx, L, ui.pointer, [
    ...
    if (ui.menu) {
      ...
      if (f && f.node.label) cursorLabel(ctx, L, ui.pointer, [{ text: f.node.label }], size, { outward: true, stroke: "#111" })
      menuLayout = { hits }
    }
