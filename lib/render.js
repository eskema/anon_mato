  ...
  function drawFrame(ctx, L, ui) {
    ...
    if (ui.menu) {
      ...
      // every discovered tile of the world, tiny, over the whole dial.
      {
        ...
        const r2 = size + (size * 3 - size) * growP
        ...
        // INSIDE A CATEGORY the tile is gone: the skill's own shape rings the
        // clock and the middle is left clear for it
        const tileA = 1 - focus01
        // ONE HEX (2026-08-10): just the grown tile — the 1:1 copy that sat
        // inside it is gone, so what you see IS your tile, larger.
        if (tileA > 0.002) tileHex(r2, tileA)
      ...
      const fanA = ui.menu.skill ? focus01 : ringIn01
      ...
      ctx.save()
      // (the white card behind the PLAYER'S cell is gone — 2026-08-10: the
      //  centre shows the tile below, as it is; only the fan's items keep
      //  their own white hexes)
      rimmed([...cells.filter(c => !c.child).map(c => c.cell)])
