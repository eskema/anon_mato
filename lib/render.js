...
//
// The view is the tile's interior plus the PERIMETER row — the neighbouring
// siblings' border tiles, which tessellate exactly with the interior (one
// continuous hex field, no silhouettes, no seam).
//
...
import { RINGS, PERIM, GATE_DIR, BASE_DEPTH, ENERGY_START, inBounds, superIndexOf } from "./sim.js"
...
// Covers the interior AND the perimeter row — one continuous field.
function buildEdges() {
  const seen = new Set()
  const out = []
  for (const [q, r] of Hex.range(PERIM)) {
    for (let d = 0; d < 6; d++) {
      const nq = q + DIRS[d].q
      const nr = r + DIRS[d].r
...
export function createRenderer(sim) {
  ...
  function sizeFor() {
    // above the base the perimeter row is part of the view — fit it too
    const ext = sim.depth() > BASE_DEPTH ? disp().extPerim : disp().ext
    return Math.min((0.45 * frame.w) / ext.hx, (0.45 * frame.h) / ext.hy)
  }
  ...
  function draw(ctx, L, ui) {
    ...
    // the field: interior + perimeter row. Explored tiles filled; scoutable
    // neighbours (perimeter included) as faint dots.
    const dots = sim.reachableDots()
    const inView = h => inBounds(h[0], h[1]) || sim.isPerimeter(h)
    for (const h of Hex.range(PERIM)) {
      const [q, r] = h
      if (!inBounds(q, r) && !sim.isPerimeter(h)) continue // sealed/off-parent lobes don't exist
      const k = `${q},${r}`
    ...
    for (const e of edges) {
      ...
      const a = [e.q, e.r]
      const b = [e.q + DIRS[e.dir].q, e.r + DIRS[e.dir].r]
      if (!inView(a) && !inView(b)) continue
      if (!sim.isDiscovered(a) && !sim.isDiscovered(b)) continue
      const c = hexToPixel(L, e.q, e.r, size)
      const cs = hexCorners(c.x, c.y, size, o.startDeg)
      const [ca, cb] = disp().edgeCorners[e.dir]
      ctx.beginPath()
    ...
    // walls (a tile prop): sealed perimeter directions drawn bold on the
    // discovered border tiles facing them — no neighbour that way.
    if (v.tile.walls) {
      ctx.strokeStyle = ink
      ctx.globalAlpha = 0.7
      ctx.lineWidth = 3
      for (const h of Hex.ring([0, 0], RINGS)) {
        ...
        for (let d = 0; d < 6; d++) {
          ...
          const lobe = superIndexOf(n[0], n[1])
          if (lobe >= 0 && !sim.walled(lobe)) continue // open crossing — no wall
    ...
    if (ui.pending) {
      ...
      const crossing = !inBounds(ui.hovered[0], ui.hovered[1])
      action = {
