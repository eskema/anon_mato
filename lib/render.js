...
import { RINGS, SEAM_RING, VIEW_RING, GATE_DIR, SEED_ANGLE, BASE_DEPTH, ENERGY_START, isSeamHex } from "./sim.js"
...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    if (v.tile.safe) {
      ...
      ctx.lineTo(frame.cx + Math.sin(rad) * len, frame.cy - Math.cos(rad) * len)
      ctx.stroke()
      ctx.globalAlpha = 1
    }
    ...
    if (v.tile.walls) {
      ...
      for (const h of Hex.ring([0, 0], RINGS)) {
        ...
        for (let d = 0; d < 6; d++) {
          ...
          // sealed unless it's the (open) gate tile — a closed gate looks like
          // wall, and the opening appears the moment the board is cleared
          if (!sim.walledAt(n)) continue
          const [ca, cb] = disp().edgeCorners[d]
          ctx.beginPath()
          ctx.moveTo(cs[ca].x, cs[ca].y)
          ctx.lineTo(cs[cb].x, cs[cb].y)
