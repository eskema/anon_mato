...
export function createRenderer(sim) {
  ...
  let cam = { x: 0, y: 0 }
  let seamView = false // true while the camera is player-centred (on seam)
  let camAnchor = null
  function setCam(anchor, size) {
    seamView = !!anchor && sim.kindOf(anchor) === "seam"
    camAnchor = anchor
    if (seamView) {
      const f = sim.orient().f
      cam = { x: size * (f[0] * anchor[0] + f[1] * anchor[1]), y: size * (f[2] * anchor[0] + f[3] * anchor[1]) }
    } else {
      cam = { x: 0, y: 0 }
    }
  }
  ...
  function draw(ctx, L, ui) {
    ...
    // the field. In the BOARD view: interior + seam + the neighbours' facing
    // rows. In the SEAM view the pass goes GLOBAL: every discovered tile in
    // the viewport draws, culled at ~4 board-pitches (or ~2 screenfuls) around
    // the player — whichever is smaller.
    ...
    if (seamView) {
      ...
      for (const d of Hex.range(rCull)) {
        const h = [camAnchor[0] + d[0], camAnchor[1] + d[1]]
        const c = hexToPixel(L, h[0], h[1], size)
      ...
      // the player's own frontier dots still apply
      for (const k of dots) {
    ...
    for (const h of seamView ? [] : Hex.range(VIEW_RING)) {
      if (!inView(h)) continue
      const [q, r] = h
      const k = `${q},${r}`
      const c = hexToPixel(L, q, r, size)
      if (sim.isDiscovered(h)) {
        // uniform fill — the trail line + arrowheads alone mark the path
        fillHex(ctx, c, size, ink, 0.05, o.startDeg)
    ...
    // walls: any discovered hex draws its walled sides bold, inward like every
    // border — a closed gate looks like wall, and the opening appears the
    // moment the board is cleared.
    {
      ctx.strokeStyle = ink
      ctx.globalAlpha = 0.7
      ctx.lineWidth = 3
      ...
      for (const h of Hex.range(VIEW_RING)) {
        if (!sim.kindOf(h) || !sim.isDiscovered(h)) continue
