...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    const drawEnergyCube = (x, y, r) => {
      ...
      // 4) the identity Y — drawn UNDER the eye, so the iris covers its top arms
      // (only the front stem stays visible, outside the lid)
      const ell = r / 2
      ctx.strokeStyle = ink
      ctx.lineWidth = W
      ctx.lineCap = "round"
      ...
      ctx.lineTo(x, y + ell)
      ctx.stroke()
      // 5) the eye — a coloured eyeball (iris + pupil + glint) that slides as ONE
      ...
      // drawn UNDER the border (step 6), so the cut tucks beneath the outline
      // instead of sitting on top. Recentres when the pointer is over the cube.
      const dCx = x
      ...
      ctx.fill()
      ctx.restore()
      // 6) the bold outline — LAST, so it caps the eye where it reaches the edge
      hex()
      ctx.strokeStyle = ink
      ctx.lineWidth = W
      ctx.lineJoin = "round"
