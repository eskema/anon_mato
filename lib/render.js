...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // the sun dial — the day's CLOCK, ringing the board. The sun's orbit sits in
    // the MIDDLE (opaque). Time is ONE continuous line — the hour, fine-grained so
    // it advances by the minute (30°/hour from 12 o'clock, so 6:00 is half a
    // circle). It starts just OUTSIDE the orbit and, over 24h, spirals to just
    // INSIDE — a perfect circle through most of a turn, curving into the next near
    // the 12h mark. A solid line is now; a ghost line runs on to where the free
    // budget (or the hovered move) lands. Advances by the minute. The dot is the
    // sun (day of the year). A fn, so it re-renders sharp over the blurred menu.
    function drawClock() {
      const R = sunDialR
      const cx = sunCentre.x
      const cy = sunCentre.y
      const TAU = Math.PI * 2
      const s = size * 0.4 // radial spread of the spiral either side of the orbit
      const at = (a, rad) => [cx + Math.sin(a) * rad, cy - Math.cos(a) * rad] // 0 = 12 o'clock, clockwise
      // the sun's orbit — opaque, in the middle
      ctx.strokeStyle = ink
      ctx.globalAlpha = 1
      ctx.lineWidth = 1.5
      ctx.beginPath()
      ...
      ctx.stroke()
      // 12 hour marks on the orbit (full alpha; quarters longer)
      ctx.lineCap = "butt"
      ctx.globalAlpha = 1
      for (let i = 0; i < 12; i++) {
        const a = (i / 12) * TAU
        const long = i % 3 === 0
        const p0 = at(a, R - (long ? size * 0.35 : size * 0.2))
        const p1 = at(a, R)
      ...
      // radius vs. angle: flat (a true circle) for most of a 12h turn, curving to
      // the next turn's radius only in its last stretch. Turn 0 outside → turn 1
      // inside, the orbit halfway between.
      const radiusAt = t => {
        const tf = t / TAU // turns elapsed (0..2 over 24h)
        const turn = Math.min(1, Math.floor(tf))
        const within = tf - turn
        const rIn = turn === 0 ? R + s : R - s
        const k = within < 0.85 ? 0 : (within - 0.85) / 0.15
        return rIn + (R - s - rIn) * k
      }
      ...
      line(now, 6, 0.85) // current time — the thick ring
      // where the day ENDS (07:00 for now = 06:00 + the full budget): a distinct
      // radial tick straddling the spiral, so you see the finish line ahead
      const ea = ((360 + ENERGY_START) / 720) * TAU
      const er = radiusAt(ea)
      const ei = at(ea, er - size * 0.3)
      const eo = at(ea, er + size * 0.3)
      ctx.strokeStyle = ink
      ctx.globalAlpha = 1
      ctx.lineWidth = 3.5
      ctx.lineCap = "round"
