...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    const RED = "#c0433a"
    // the trail is an INK (text-colour) border silhouette with a SURFACE line on
    // top — a background-coloured stroke outlined in the text colour. An illegal
    // hover swaps the border to red; the line stays surface either way.
    const hovBorder = bad ? RED : ink
    let ta = null
    let tb = null
    if (hov && hov.length >= 2) {
    ...
    // pass 1 — the ink border silhouette, under everything (always solid)
    strokePath(ctx, L, size, trail, ink, 4.5, 0.9)
    strokePath(ctx, L, size, hov, hovBorder, 4.5, 0.9)
    trailArrows(ctx, L, size, trail, ink, 7, 0.9)
    if (tb) {
      if (bad) crossTip(ctx, tb.x, tb.y, RED, size, 6, 0.9)
      else {
        ctx.globalAlpha = 0.9
        arrowTip(ctx, ta.x, ta.y, tb.x, tb.y, ink, size * 0.32, size * 0.2, 7)
        ctx.globalAlpha = 1
      }
    }
    // pass 2 — the surface lines + heads on top
    strokePath(ctx, L, size, trail, surface, 1.5, 0.9)
    strokePath(ctx, L, size, hov, surface, 1.5, 0.9, [5, 5])
    trailArrows(ctx, L, size, trail, surface, 1.5, 0.9)
    if (tb) {
      if (bad) crossTip(ctx, tb.x, tb.y, surface, size, 2, 0.95)
      else {
        ctx.globalAlpha = 0.9
        arrowTip(ctx, ta.x, ta.y, tb.x, tb.y, surface, size * 0.32, size * 0.2, 1.5)
        ctx.globalAlpha = 1
      }
    }
