...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    const drawEnergyCube = (x, y, r) => {
      ...
      // 1) the cast shadow — a REAL floor projection, not an offset box: shear
      // the cube's silhouette along the ground AWAY from the sun (the −toward-sun
      // direction, so it runs down the sun→cube line), each vertex pushed by its
      // height (bottom point anchored on the floor, top swung furthest). Long
      // when the sun is low (sunLen), tinted by the day's hue, softened so it
      // reads as lying on the tiles. Drawn UNDER the cube, so only the part that
      // escapes the base shows.
      const sdx = -tsx * sunLen
      const sdy = -tsy * sunLen
      const silo = [
        [x, y - r, 1], // top peak — highest, projects furthest
        [x + w, y - h, 0.75],
        [x + w, y + h, 0.25],
        [x, y + r, 0], // bottom point — on the floor, stays put
        [x - w, y + h, 0.25],
        [x - w, y - h, 0.75]
      ]
      ctx.save()
      ...
      ctx.beginPath()
      silo.forEach((p, i) => {
        const sx = p[0] + p[2] * sdx
        const sy = p[1] + p[2] * sdy
        i ? ctx.lineTo(sx, sy) : ctx.moveTo(sx, sy)
      })
      ctx.closePath()
      ctx.fillStyle = `hsla(${sunDeg}, 45%, 26%, 0.5)`
      ctx.fill()
      ctx.restore()
