...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // board; on the seam it rings the walker), doubling as the day's CLOCK. The
    // outer ring carries hour ticks; two hands show the current time (06:00 +
    // spent), and a second, ghosted pair shows where the clock lands when the
    // free budget runs out — or, while hovering a tile, right after that move.
    // The dot is still the sun: one step per day, wearing the day's hue.
    {
      const R = sunDialR
      ...
      // clock hands from the centre. hour + minute, one pair per time.
      const clockAt = min => {
        const hr = 6 + Math.floor(min / 60)
        const mn = ((min % 60) + 60) % 60
        return { hour: (((hr % 12) + mn / 60) / 12) * Math.PI * 2, minute: (mn / 60) * Math.PI * 2 }
      }
      const hands = (t, alpha) => {
        ctx.strokeStyle = ink
        ctx.lineCap = "round"
        ...
        ctx.beginPath()
        ctx.moveTo(cx, cy)
        ctx.lineTo(...at(t.hour, R * 0.5))
        ctx.stroke()
        ctx.lineWidth = 2
        ctx.beginPath()
        ctx.moveTo(cx, cy)
        ctx.lineTo(...at(t.minute, R * 0.8))
        ctx.stroke()
      }
      ...
      hands(clockAt(spent + cost), 0.18) // the ghost pair (run-out / after the move), under
      hands(clockAt(spent), 0.5) // the current time, on top
