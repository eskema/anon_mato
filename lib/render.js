...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // the player cube — and the figures NESTED above it, all concentric.
    // Exact subdivision sizes: split each of the tile hexagon's six wedges
    // into 4 triangles and the radius midpoints reconstruct a half-size hex
    // inside, same orientation. So: tile (R) ⊃ player (R/2) ⊃ figure (R/4) —
    // drawCube takes the "tile width", whose corner radius is half of it.
    const pc = hexToPixel(L, cubeTile[0], cubeTile[1], size)
    castShadow(true)
    drawCube(ctx, pc.x, pc.y, size, ink, surface, o.startDeg)
    castShadow(false)
    // the resting figures on top — half the player's cube, dead centre
    for (const c of centres) {
      if (!c.npc) continue
      drawCube(ctx, c.cp.x, c.cp.y, size * 0.5, ink, surface, o.startDeg)
    }
