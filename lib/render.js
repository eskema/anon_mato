...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    const drawEnergyCube = (x, y, r) => {
      ...
      ctx.lineTo(x, y + ell)
      ctx.stroke()
      // 6) the eye — a white sclera on the TOP FACE with a pupil that rolls
      // toward the pointer. The pupil is clipped to the sclera (and the cube), so
      // when it looks to the side it slides under the rim and gets cut — a
      // googly-eye effect. It recentres whenever the pointer is over the cube.
      const eCx = x
      const eCy = y - r / 2 // the lid's centre
      const scleraR = r * 0.36
      const pupilR = r * 0.22
      const travel = scleraR - pupilR * 0.4 // far enough that the pupil cuts on the rim
      let ox = 0
      let oy = 0
      if (ui.pointer && Math.hypot(ui.pointer.x - x, ui.pointer.y - y) >= r) {
        const dx = ui.pointer.x - eCx
        const dy = ui.pointer.y - eCy
        const m = Math.hypot(dx, dy) || 1
        const t = Math.min(travel, m)
        ox = (dx / m) * t
        oy = (dy / m) * t
      }
      ctx.save()
      hex()
      ctx.clip() // the eye sits IN the face — its rim cut by the cube silhouette
      ctx.beginPath()
      ctx.arc(eCx, eCy, scleraR, 0, Math.PI * 2)
      ctx.fillStyle = surface
      ctx.fill()
      ctx.lineWidth = W * 0.6
      ctx.strokeStyle = ink
      ...
      ctx.beginPath()
      ctx.arc(eCx, eCy, scleraR, 0, Math.PI * 2)
      ctx.clip() // the pupil rolls under the sclera rim and gets cut there
      ctx.beginPath()
      ctx.arc(eCx + ox, eCy + oy, pupilR, 0, Math.PI * 2)
      ctx.fillStyle = ink
      ctx.fill()
      ctx.restore()
    }
