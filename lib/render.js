  ...
  function draw(ctx, L, ui) {
    ...
    const dots = sim.reachableDots()
    {
      ...
      for (const dlt of Hex.range(rCull)) {
        ...
        if (kind === "seam") {
        ...
        } else if (sim.nibbleAt(h) != null) {
          // the raw-key view (dev): the tile IS its hex character(s). Six
          // wedges, each a gradient from the tile's own grey at the centre
          // toward the shared-edge MIDPOINT value with that side's neighbour
          // (both sides meet at the same value, so tiles smooth into each
          // other). Fog/seam sides stay flat — no leaking undiscovered chars.
          const ch = sim.nibbleAt(h)
          const val = s => [...s].reduce((t, c2) => t + parseInt(c2, 16), 0) / s.length
          const grey = v => {
            const b2 = Math.round(v * 17)
            return `rgb(${b2},${b2},${b2})`
          }
          const v0 = val(ch)
          const cf = hexCorners(c.x, c.y, size, o.startDeg)
          // neighbour values (null = fog/seam — contribute nothing, no leaks)
          const vn = []
          for (let d = 0; d < 6; d++) {
            const n = [h[0] + DIRS[d].q, h[1] + DIRS[d].r]
            const nch = sim.isDiscovered(n) ? sim.nibbleAt(n) : null
            vn[d] = nch != null ? val(nch) : null
          }
          // perimeter samples: edge midpoints meet the neighbour halfway;
          // corners average the (up to) three tiles that share them
          const cornerEdges = [...Array(6)].map(() => [])
          for (let d = 0; d < 6; d++) for (const ci of disp().edgeCorners[d]) cornerEdges[ci].push(d)
          const samples = []
          for (let d = 0; d < 6; d++) {
            const [wa, wb] = disp().edgeCorners[d]
            samples.push({
              x: (cf[wa].x + cf[wb].x) / 2,
              y: (cf[wa].y + cf[wb].y) / 2,
              v: vn[d] != null ? (v0 + vn[d]) / 2 : v0
            })
          }
          for (let ci = 0; ci < 6; ci++) {
            const vs = [v0, ...cornerEdges[ci].map(d => vn[d]).filter(v => v != null)]
            samples.push({ x: cf[ci].x, y: cf[ci].y, v: vs.reduce((a, b) => a + b, 0) / vs.length })
          }
          // one conic sweep through the 12 perimeter values (canvas y-down
          // makes atan2 run clockwise, matching the conic direction)…
          for (const s of samples) s.f = (Math.atan2(s.y - c.y, s.x - c.x) / (2 * Math.PI) + 1) % 1
          samples.sort((a, b) => a.f - b.f)
          const first = samples[0]
          const last = samples[samples.length - 1]
          const t = (1 - last.f) / (first.f + 1 - last.f)
          const vWrap = last.v + (first.v - last.v) * t
          const conic = ctx.createConicGradient(0, c.x, c.y)
          conic.addColorStop(0, grey(vWrap))
          for (const s of samples) conic.addColorStop(s.f, grey(s.v))
          conic.addColorStop(1, grey(vWrap))
          fillHex(ctx, c, size, conic, 1, o.startDeg)
          // …then the tile's own value fades out from the centre over it
          const b0 = Math.round(v0 * 17)
          const radial = ctx.createRadialGradient(c.x, c.y, 0, c.x, c.y, size)
          radial.addColorStop(0, `rgba(${b0},${b0},${b0},1)`)
          radial.addColorStop(1, `rgba(${b0},${b0},${b0},0)`)
          fillHex(ctx, c, size, radial, 1, o.startDeg)
          const avg = Math.round(v0)
          ctx.fillStyle = avg >= 8 ? "#1b1b1b" : "#eeeeee"
          ctx.textAlign = "center"
          ctx.textBaseline = "middle"
          ctx.globalAlpha = 0.9
          if (ch.length === 1) {
            ctx.font = `600 ${Math.round(size * 0.55)}px ui-monospace, monospace`
            ctx.fillText(ch, c.x, c.y)
          } else {
            // the centre holds the key's middle four — a 2×2 block
            ctx.font = `600 ${Math.round(size * 0.34)}px ui-monospace, monospace`
            const dx = size * 0.21
            const dy = size * 0.24
            ctx.fillText(ch[0], c.x - dx, c.y - dy)
            ctx.fillText(ch[1], c.x + dx, c.y - dy)
            ctx.fillText(ch[2], c.x - dx, c.y + dy)
            ctx.fillText(ch[3], c.x + dx, c.y + dy)
          }
          ctx.globalAlpha = 1
          ...
          // the derived terrain: biome colour straight from the world key
          // (keyless dev worlds fall through to the plain ink look)
