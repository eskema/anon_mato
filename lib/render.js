...
export function createRenderer(sim) {
  ...
  function labelLayout(ctx, L, pointer, lines, tile) {
    ...
    // the box is centred along that direction, far enough that its near edge is
    // always `gap` away. Continuous (the direction rotates smoothly across the
    // axes, no corner jumps), then clamped as a safety net.
  ...
  // thin white leader lines from the cursor to EVERY corner of every line box —
  // a little fan. The boxes carry a matching white border, so the lines flow into
  // the corners. Drawn UNDER the hover trail (and the boxes).
  function labelFan(ctx, lay) {
    if (!lay) return
  ...
  function labelBox(ctx, lay) {
    ...
    lines.forEach((l, i) => {
      ...
      ctx.strokeStyle = "#fff" // matching border with rounded corners, for the fan
      ctx.lineWidth = 2
  ...
  function cursorLabel(ctx, L, pointer, lines, tile) {
    const lay = labelLayout(ctx, L, pointer, lines, tile)
    labelFan(ctx, lay)
    labelBox(ctx, lay)
  }
