...
export function createRenderer(sim) {
  ...
  function cursorLabel(ctx, L, pointer, lines, tile) {
    ...
    // a white leader line from the cursor to the CLOSEST edge-centre of any line's
    // background box — meets the label mid-edge, not at a corner or its centre.
    ...
    lines.forEach((l, i) => {
      const y = top + i * lineH
      const midX = left + widths[i] / 2
      const midY = y + lineH / 2
      const edges = [
        [midX, y], // top
        [midX, y + lineH], // bottom
        [left, midY], // left
        [left + widths[i], midY] // right
      ]
