...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    const drawEnergyCube = (x, y, r) => {
      ...
      ctx.lineTo(x, y + ell)
      ctx.stroke()
      // 6) the eye — a pupil sitting on the TOP FACE (its socket). It slides
      // within the lid toward the pointer, clamped to the lid's diamond, and
      // recentres whenever the pointer is over the cube itself.
      const eyeR = r * 0.26
      const eCx = x
      ...
      if (ui.pointer && Math.hypot(ui.pointer.x - x, ui.pointer.y - y) >= r) {
        ...
        const tw = Math.max(1, w - eyeR - W) // travel room inside the lid diamond
        const th = Math.max(1, r / 2 - eyeR - W)
        const denom = Math.abs(dx) / tw + Math.abs(dy) / th
        const s = denom > 1 ? 1 / denom : 1 // ride the lid's edge, never past it
        ex = dx * s
        ey = dy * s
      }
      ctx.beginPath()
      ctx.arc(eCx + ex, eCy + ey, eyeR, 0, Math.PI * 2)
      ctx.fillStyle = ink
      ctx.fill()
    }
