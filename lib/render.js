...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    const drawEnergyCube = (x, y, r) => {
      ...
      ctx.lineTo(x, y + ell)
      ctx.stroke()
      // 6) the eye — a coloured eyeball (an iris disc + dark pupil) that slides as
      // ONE toward the pointer within the TOP FACE. Its socket is the lid DIAMOND;
      // the eyeball is bigger than fits, so the diamond cuts it as it rolls to an
      // edge, seating it inside the face. Recentres when the pointer is over the
      // cube. The iris is a full-value shade of the angle colour, no border.
      const dCx = x
      const dCy = y - r / 2 // the lid's centre
      const irisR = r * 0.34
      const pupilR = r * 0.17
      const maxTravel = r * 0.38
      let ox = 0
      let oy = 0
