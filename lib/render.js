...
export function createRenderer(sim) {
  ...
  function labelLayout(ctx, L, pointer, lines, tile, outward = false, keepClear = null) {
    ...
    return { lines, left: best.left, top: best.top, lineH, pad, widths, pointer }
  }
  ...
  function draw(ctx, L, ui) {
    ...
    const reserved = ui.pending?.ghostTile ? sim.returnFrom(ui.pending.ghostTile) : sim.returnCost()
    ...
    function drawClock() {
      ...
      // a spiral SEGMENT [fromMin, toMin] (minutes since midnight), advancing by
      // the minute, at radiusAt(t) + `off` (off > 0 pushes it flush to the band's
      // OUTER edge; 0 rides the centre)
      const seg = (fromMin, toMin, width, alpha, off = 0) => {
        const s0 = (Math.floor(fromMin) / 720) * TAU
        const s1 = (Math.floor(toMin) / 720) * TAU
        if (s1 - s0 <= 0.0001) return
        ctx.strokeStyle = ink
        ctx.globalAlpha = alpha
        ctx.lineWidth = width
        ctx.lineCap = "butt"
        ctx.lineJoin = "miter"
        ctx.beginPath()
        const steps = Math.max(2, Math.ceil((s1 - s0) / 0.04))
        for (let i = 0; i <= steps; i++) {
          const t = s0 + (s1 - s0) * (i / steps)
          const p = at(t, radiusAt(t) + off)
          i ? ctx.lineTo(p[0], p[1]) : ctx.moveTo(p[0], p[1])
        }
        ctx.stroke()
      }
      const now = 360 + spent // minutes since midnight (06:00 + spent)
      const end = 360 + ENERGY_START // 07:00 — the hard limit
      seg(0, now, bandW, 0.85) // current time — the thick band, centred
      // the hovered move (its cost, from now) and the RETURN TRIP home (its
      // reserve, held at the day's end), both thin and flush to the band's OUTER
      // edge. Showing the return is why the free gap between them reads true.
      const outW = 3
      const outOff = bandW / 2 - outW / 2
      let cost = 0
      let ret = sim.returnCost() // reserve from where we stand (the walk to the nearest rest spot)
      if (!ui.pending && ui.hovered && ui.hoverPath && !ui.hoverIllegal) {
        cost = sim.pathCharge(ui.hoverPath)
        ret = sim.returnFrom(ui.hovered)
      }
      if (!isFinite(ret)) ret = 0
      seg(now, now + cost, outW, 0.5, outOff) // the hovered move
      seg(end - ret, end, outW, 0.3, outOff) // the return trip, held at the finish
    ...
    if (ui.pointer && !ui.menu && !ui.pending && ui.hovered) {
      ...
      if (ui.hoverPath) {
        ...
        l1 = `+${Math.round(sim.pathCharge(ui.hoverPath))} ${crossing ? "crossing to" : "walking to"}`
      } else if (sim.isFrontier(h) && sim.canScout(h)) {
        l1 = `+${Math.round(sim.scoutChargeAt(h))} scouting`
      }
      if (l1)
    ...
    // full-black with arrowheads pointing home — the return trail (when away)
    const home = sim.homePath()
    if (home && home.length >= 2) {
      strokePath(ctx, L, size, home, ink, 1.5, 1)
      trailArrows(ctx, L, size, home, ink, 1.5, 1)
    }
    ...
    const drawEnergyCube = (x, y, r) => {
      ...
      if (!ui.pending && ui.hovered && ui.hoverPath) {
        mins = Math.round(freeEnergy - sim.pathCharge(ui.hoverPath))
        faded = true
      } else if (!ui.pending && ui.hovered && sim.isFrontier(ui.hovered) && sim.canScout(ui.hovered)) {
        mins = Math.round(freeEnergy - sim.scoutChargeAt(ui.hovered))
        faded = true
      }
      ctx.fillStyle = ink
