...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // the people: each board's figure at its centre, visible once that tile
    // is discovered (styling TBD — a smaller, quieter cube for now)
    for (const bc of Hex.range(RINGS)) {
      const npc = sim.npcAt(bc)
      if (!npc || !sim.isDiscovered(npc.pos)) continue
      const p = hexToPixel(L, npc.pos[0], npc.pos[1], size)
      if (p.x < -size || p.x > L.w + size || p.y < -size || p.y > L.h + size) continue
      drawCube(ctx, p.x, p.y, size * 0.8, ink, surface, o.startDeg, false, 0.75)
    }
    ...
    // The controller owns the spec + open/focus state; we assign cells,
    // draw a padded tile silhouette behind them, then the icon hexes.
    menuLayout = null
    if (ui.menu) {
      const P = v.player
      const pc = hexToPixel(L, P[0], P[1], size)
      ...
      if (them.length && ui.menu.themPos) {
        // split: them toward the figure, self on the far cells
        const tpx = hexToPixel(L, ui.menu.themPos[0], ui.menu.themPos[1], size)
        const themA = (Math.atan2(tpx.x - pc.x, -(tpx.y - pc.y)) + Math.PI * 2) % (Math.PI * 2)
        const toward = ring1.slice().sort((a, b) => angDiff(ang(a), themA) - angDiff(ang(b), themA))
        const themCells = toward.slice(0, them.length)
        const selfCells = toward.slice(them.length).reverse() // remaining, farthest first
        them.forEach((n, i) => themCells[i] && cells.push({ node: n, cell: themCells[i] }))
        self.forEach((n, i) => selfCells[i] && cells.push({ node: n, cell: selfCells[i] }))
      } else {
      ...
      // background: a padded silhouette — every occupied cell (player too) as
      // an oversized hex, all in ONE path filled once (no overlap seams) so
      // the union reads as one menu shape with ~half a tile of padding
      ctx.save()
      ctx.fillStyle = surface
      ctx.globalAlpha = 0.9
      ctx.beginPath()
      for (const cellKey of used) {
        ...
        const cp = hexToPixel(L, cq, cr, size)
        const cs = hexCorners(cp.x, cp.y, size * 1.5, o.startDeg)
      ...
      // the player at the hub, on top of the silhouette
      drawCube(ctx, pc.x, pc.y, size, ink, surface, o.startDeg)
      ...
      for (const c of cells) {
        ...
        ctx.stroke()
        ctx.globalAlpha = 1
        drawIcon(ctx, c.node.icon, p.x, p.y, size * 0.42, open2 ? surface : ink, on ? 0.95 : 0.4)
