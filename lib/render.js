...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // a copy of the hover info pinned near the cursor: two lines — cost + action,
    // then the coordinates — each on a snug white background (no border/rounding).
    // Offset away from the trail (outward, past the hovered tile), then clamped so
    ...
    if (ui.pointer && !ui.menu && !ui.pending && ui.hovered && action) {
      ...
      // offset direction: outward along the trail (player → hovered), so the label
      // sits on the far side of the cursor from the trail
      const hp = hexToPixel(L, h[0], h[1], size)
      const pp = hexToPixel(L, v.player[0], v.player[1], size)
      const dl = Math.hypot(hp.x - pp.x, hp.y - pp.y) || 1
      const dx = (hp.x - pp.x) / dl
      const dy = (hp.y - pp.y) / dl
      ctx.font = "600 16px system-ui, sans-serif"
      const pad = 3
      const lineH = 21
      const w1 = ctx.measureText(l1).width + pad * 2
      const w2 = ctx.measureText(l2).width + pad * 2
      const boxW = Math.max(w1, w2)
      const boxH = lineH * 2
      const off = 16
      let left = dx >= 0 ? ui.pointer.x + dx * off : ui.pointer.x + dx * off - boxW
      let top = dy >= 0 ? ui.pointer.y + dy * off : ui.pointer.y + dy * off - boxH
      left = Math.max(4, Math.min(L.w - boxW - 4, left))
      top = Math.max(4, Math.min(L.h - boxH - 4, top))
      ctx.globalAlpha = 1
      ctx.fillStyle = "#fff"
      ctx.fillRect(left, top, w1, lineH)
      ctx.fillRect(left, top + lineH, w2, lineH)
      ctx.textAlign = "left"
      ctx.textBaseline = "middle"
      ctx.fillStyle = ui.hoverIllegal ? "#c0433a" : "#111"
      ctx.fillText(l1, left + pad, top + lineH / 2)
      ctx.fillStyle = "#111"
      ctx.globalAlpha = 0.6
      ctx.fillText(l2, left + pad, top + lineH + lineH / 2)
      ctx.globalAlpha = 1
      ctx.textAlign = "left"
    }
