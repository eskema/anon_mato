...
export function createRenderer(sim) {
  ...
  function labelLayout(ctx, L, pointer, lines, tile, outward = false, keepClear = null) {
    ...
    // sit a constant gap (~half a tile) off the cursor toward the viewport centre
    // (or AWAY from it when `outward` — the menu pushes its label off the player
    // and the tile, which both sit at centre while it's open). The box is centred
    // along that direction, its near edge always `gap` away; continuous across the
    // axes, then clamped.
    const s = outward ? -1 : 1
