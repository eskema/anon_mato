...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // once). Elements cast tiny shadows away from it.
    const sunDeg = (sim.day() - 1) % 360
    const sunRad = (sunDeg * Math.PI) / 180
    const castShadow = on => {
      if (on) {
        ctx.shadowColor = "rgba(0,0,0,0.35)"
        ctx.shadowOffsetX = -Math.sin(sunRad) * 2
        ctx.shadowOffsetY = Math.cos(sunRad) * 2
        ctx.shadowBlur = 1
      } else {
    ...
    // the sun dial: a small circle top-centre below the clock, always in
    // view — the year made visible. The dot steps one degree per day and
    // wears the day's hue; no globe, just a hint of the mechanic.
    {
      const dial = { x: L.w / 2, y: L.h - frame.h + 30, r: 14 }
      ctx.strokeStyle = ink
      ...
      ctx.beginPath()
      ctx.arc(dial.x + Math.sin(sunRad) * dial.r, dial.y - Math.cos(sunRad) * dial.r, 3.5, 0, Math.PI * 2)
      ctx.fill()
    }
