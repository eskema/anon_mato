...
import { layoutRadial, drawRadial, hitRadial, drawStatsPanel } from "./radial.js"
...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    // the radial menu — a folder tree fanned around the player over a soft
    // disc (see radial.js). The controller owns the spec + open/focus state;
    // here we lay it out (finding the faced figure's screen angle for the
    // split), draw it, and stash the hit list for the controller.
    menuLayout = null
    if (ui.menu) {
      const c = hexToPixel(L, v.player[0], v.player[1], size)
      let themAngle = -Math.PI / 2
      if (ui.menu.them?.length && ui.menu.themPos) {
        const tp = hexToPixel(L, ui.menu.themPos[0], ui.menu.themPos[1], size)
        themAngle = Math.atan2(tp.y - c.y, tp.x - c.x)
      }
      menuLayout = layoutRadial({ cx: c.x, cy: c.y, size, spec: ui.menu, openId: ui.menu.openId, themAngle })
      drawRadial(ctx, L, menuLayout, { ink, surface, focusId: ui.menu.focusId, cx: c.x, cy: c.y })
    }
  ...
  return {
    ...
    menuHit: p => (menuLayout ? hitRadial(menuLayout, p) : null)
  }
}
