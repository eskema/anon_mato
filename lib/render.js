...
export function createRenderer(sim) {
  ...
  function draw(ctx, L, ui) {
    ...
    const drawEnergyCube = (x, y, r) => {
      ...
      // the cube's silhouette along the ground away from the sun, each vertex
      // pushed by its height (bottom point anchored on the floor, top swung
      // furthest). Long when the sun is low (sunLen), tinted by the day's hue,
      // softened so it reads as lying on the tiles. Drawn UNDER the cube, so
      // only the part that escapes the base shows.
      const sdx = -Math.sin(sunRad) * sunLen
      const sdy = Math.cos(sunRad) * sunLen
      const silo = [
