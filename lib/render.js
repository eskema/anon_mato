    ...
    const dots = sim.reachableDots()
    {
      ...
      for (const dlt of Hex.range(rCull)) {
        ...
        if (kind === "seam") {
          ...
          // hex character — #ccc-style grey straight from the char, and the
          // character printed at the centre
          const ch = sim.nibbleAt(h)
          fillHex(ctx, c, size, `#${ch}${ch}${ch}`, 1, o.startDeg)
          ctx.fillStyle = parseInt(ch, 16) >= 8 ? "#1b1b1b" : "#eeeeee"
