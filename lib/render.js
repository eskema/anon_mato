...
export function createRenderer(sim) {
  ...
  // of dark text on snug full-white backgrounds (no border/rounding), pinned near
  // the cursor. Placed BETWEEN the cursor and the viewport centre (a short hop, so
  // it stays close to the cursor and naturally on-screen), then clamped. `lines`
  // is [{ text, color?, alpha? }].
  function cursorLabel(ctx, L, pointer, lines, tile) {
    ...
    // sit a constant gap (~half a tile of clear space) off the cursor, toward the
    // viewport centre: the box is centred along that direction, far enough that
    // its near edge is always `gap` from the cursor. Fully continuous — the
    // direction rotates smoothly as the cursor crosses an axis, no corner jumps —
    // then clamped to the viewport as a safety net.
    let dx = L.cx - pointer.x
    let dy = L.cy - pointer.y
    ...
    // a white leader line from the cursor to the CLOSEST corner of any line's
    // background box. The boxes carry a matching white border (below), so the
    // line flows into it and never looks dislocated at the corner. Drawn under
    // the backgrounds.
    let anchorX = left
    let anchorY = top
    let bestD = Infinity
    lines.forEach((l, i) => {
      const y = top + i * lineH
      for (const gx of [left, left + widths[i]]) {
        for (const gy of [y, y + lineH]) {
          const d = (gx - pointer.x) ** 2 + (gy - pointer.y) ** 2
          if (d < bestD) {
            bestD = d
            anchorX = gx
            anchorY = gy
    ...
    lines.forEach((l, i) => {
      ...
      // a matching white border (same width as the leader) with rounded corners,
      // so the leader flows into the corner instead of poking at a bare point
      ctx.strokeStyle = "#fff"
      ctx.lineWidth = 2
  ...
  function draw(ctx, L, ui) {
    ...
    // The dot is the sun: one step per day, wearing the day's hue.
    {
      ...
      ctx.lineWidth = 1.5
      ctx.beginPath()
      ctx.arc(sunCentre.x, sunCentre.y, sunDialR, 0, Math.PI * 2)
      ctx.stroke()
      ctx.globalAlpha = 1
    ...
    if (ui.menu) {
      ...
      if (f && f.node.label) cursorLabel(ctx, L, ui.pointer, [{ text: f.node.label }], size)
      menuLayout = { hits }
    }
    ...
    // the hover info, pinned near the cursor: cost + action, then the coordinates
    // (see cursorLabel — the same space every hover readout uses)
    if (ui.pointer && !ui.menu && !ui.pending && ui.hovered && action) {
      const h = ui.hovered
