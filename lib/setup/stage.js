...
import { createRenderer, cursorDot, dialRadius, drawPlayer, playerWeight, frontierDot, fogHover, fogCoat, hexCorners, mapLight, nightPair } from "../render.js"
...
export function createStage(pubkey = null) {
  ...
  const onCentre = p => Math.hypot(p.x - geom.x, p.y - geom.y) <= geom.size * 0.87
  ...
  function drawWake(ctx, alpha = 1) {
    ...
    ctx.fill()
    ctx.globalAlpha = 1
    drawIcon(ctx, "wake", geom.x, geom.y, geom.size / 0.9, ink, 0.95 * alpha)
  }
  ...
  return { place, night, dialR, geom: () => geom, onCentre, drawCentre, drawFigure, drawWake, label, chrome, overChrome, cursor }
}
