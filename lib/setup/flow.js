...
// Mounts the setup flow onto the app: wires commit handling and shows the first
// intake screen. Call this to run setup; leaving it uncalled keeps setup stashed.
export function startSetup(app) {
  let stepIndex = 0
  app.onCommit(value => {
    SETUP[stepIndex].apply(app.store, value)
    stepIndex++
    if (stepIndex < SETUP.length) app.setScreen(SETUP[stepIndex].screen())
    else finishSetup(app.store)
  })
  app.setScreen(SETUP[0].screen())
}

// Called once every setup step is committed. The current screen stays mounted
// and shows its own committed state, so this just records the hand-off — later
// it seeds the actual game/environment from the derived store.
function finishSetup(store) {
  console.log("[thrive] setup complete", store)
}
