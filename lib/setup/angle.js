...
const T_SETTLE = 760 // …and the corners draw in to the tiles beside you
...
const ENDED = DONE + T_SETTLE
// the wheel lands MID-TURN — half a step off the corners — so its six dots sit
// on the six NEIGHBOUR directions (30° + k·60), which is where they have to be
// to draw in and become the discovery dots on the tiles beside you.
const END_OFF = 30
const FRONTIER_DOT = 2.5 // …and the size they end at: the game's own frontier mark
...
export function AngleScreen({ centreDot = true, centre = null, radius = null, tile = null, home = null } = {}) {
  ...
  const turn = e => (e <= AT_ROT ? 0 : -(angle + 360 - END_OFF) * easeOutQuart(span(e, AT_ROT, DONE)))
  function wheel(ctx, c, r, t, ink, A) {
    ...
    // THE SETTLE (2026-08-28): the turn over, the six corners draw INWARD off
    // the horizon and shrink, coming to rest at the centres of the tiles
    // beside you — at the size of the game's own discovery dots. Their lines
    // come with them and carry on to the middle, so the spokes stay whole.
    const settle = easeOutQuart(span(e, DONE, ENDED))
    const rr = r + (Math.sqrt(3) * t - r) * settle
    const dr = dot + (FRONTIER_DOT - dot) * settle
    // …and the SPOKES go with them, all the way into nothing (2026-08-28):
    // over the settle each line is eaten from its outer end inward, so when the
    // dots come to rest on the tiles beside you there is no line left at all —
    // only the six marks, exactly as the game's own discovery dots stand.
    const lineOut = 1 - settle
    for (let k = 0; k < 6; k++) {
      const p = along(c, angle + rot + k * 60, rr)
      // the spoke first, so the corner sits on top of its own line
      if (grow > 0 && lineOut > 0.002)
        stroke(
          ctx,
          [
            { x: lerp(c.x, p.x, lineOut), y: lerp(c.y, p.y, lineOut) },
            { x: lerp(p.x, c.x, grow), y: lerp(p.y, c.y, grow) }
          ],
          ink,
          1,
          A * lineOut
      ...
      ctx.beginPath()
      ctx.arc(p.x, p.y, dr * easeOutQuart(pop), 0, Math.PI * 2)
      ctx.fillStyle = ink
      ctx.globalAlpha = A
      ctx.fill()
    }
  ...
  function draw(ctx, L, dress = null) {
    ...
    if (set && !struck && atHome(L)) {
      const u = (performance.now() % 1400) / 1400
      const rp = REM * 0.75 + REM * 0.85 * u
      ctx.beginPath()
      ctx.arc(c.x, c.y, rp, 0, Math.PI * 2)
      ctx.strokeStyle = ink
      ctx.globalAlpha = A * 0.6 * (1 - u) // …fading as it opens, so it breathes rather than blinks
      ctx.lineWidth = STROKE * (1 - u * 0.75) // …and thinning as it goes, so it dissolves rather than stops
      ctx.stroke()
      ctx.globalAlpha = 1
    }
    ...
    if (!set) {
      ...
      // THE ANGLE'S LINE STAYS (2026-08-28): the leg you walked, its ray and
      // its arrowheads used to fade out the moment the wheel began to turn.
      // The angle is what the ceremony is about — it holds for all of it.
      const W = A
      ...
      if (pointer && !struck) {
        ...
        const a = HOME_A * A
        if (home) {
          // BACK AT THE CENTRE (2026-08-10): the way home used to fade to
          // nothing here — you arrived and the path vanished. It stays: the
          // connecting line itself, wearing its own arrow pointing BACK, set
          // a third of the way in so it never collides with the outward
          // arrowhead at the leg's middle.
          stroke(ctx, [P, c], ink, HOME_W, a)
          ctx.globalAlpha = a
          arrowTip(ctx, P.x, P.y, P.x + (c.x - P.x) * 0.35, P.y + (c.y - P.y) * 0.35, ink, t * 0.32, t * 0.2, HOME_W)
          ctx.globalAlpha = 1
        } else {
  ...
  return {
    ...
    animating: () => (!!struck && since() < ENDED) || opening() || (set && !struck && !!pointer && !!lastL && atHome(lastL)),
    ...
    // the sky comes in over the big turn — there by the time the wheel lands
    skyIn: () => (struck ? span(since(), AT_ROT, DONE) : 0),
