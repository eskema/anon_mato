...
export function ConfirmScreen(store) {
  ...
  function onPointerDown(p) {
    ...
    if (picker.ready()) return api.commit({ resume: false, angle })
  ...
  function keyRows() {
    const rows = [{ key: "pubkey", line: { text: pubkey } }, { key: "npub", line: { text: npub }, delay: NPUB_DELAY }]
    if (name !== undefined) {
      rows.push({
        key: "who",
        line: {
          cells: [
            { text: name === false ? "(no profile found)" : name },
            { text: `relays ${show(relays)}`, alpha: 0.6 },
            { text: `follows ${show(follows)}`, alpha: 0.6 }
          ]
        }
      })
    }
  ...
  function draw(ctx, L) {
    ...
    if (picker.ready()) stage.drawWake(ctx)
