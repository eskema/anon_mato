...
export function createSim() {
  ...
  // One BFS sweep from the player over discovered ground, recording each
  // tile's route charge (same min-hop tree bfsPathDisc walks, so the charge
  // matches the path a move would actually take). canMove becomes a lookup —
  // hover and the fuzzers hammer it for many targets from the same spot.
  let reachCache = { stamp: -1, from: "", charge: null }
  function reachCharges() {
    const fk = key(view().player)
    if (reachCache.stamp === worldStamp && reachCache.from === fk) return reachCache.charge
    const charge = new Map([[fk, 0]])
    const queue = [view().player]
    while (queue.length) {
      const cur = queue.shift()
      const c = charge.get(key(cur))
      for (const n of moveNeighbors(cur)) {
        const k = key(n)
        if (charge.has(k) || !isDiscovered(n)) continue
        charge.set(k, c + stepChargeAt(n))
        queue.push(n)
      }
    }
    reachCache = { stamp: worldStamp, from: fk, charge }
    return charge
  }

  function canMove(target) {
    if (!kindOf(target) || !isDiscovered(target)) return false
    const c = reachCharges().get(key(target))
    if (c === undefined) return false
    const reserve = freeAt(target) ? 0 : returnFrom(target)
    return c + reserve <= energy
  }
