...
export const SCHEMA = 2 // save format version — the shape of the serialized object (2: world.pubkey)
export const RULES = 1 // replay-rules version — bump on ANY change that alters what an old
...
export const TILE_TYPES = {
  ...
  // derived land (the pubkey inscription, v0). Inside the safe home these
  // charge nothing anyway — flavour first; the multipliers bite only when
  // types later appear on unsafe ground. Capped at 2× (variance reads as
  // flavour at 2×, as punishment beyond).
  meadow: { move: 1, scout: 1 },
  grove: { move: 1.5, scout: 1 },
  marsh: { move: 2, scout: 1 },
  crag: { move: 2, scout: 2 },
  spring: { move: 1, scout: 0.5 } // the future resting-place hook
}

// ── world derivation (v0: the key inscribed on the home board) ──────
// A nostr pubkey is 32 bytes = 64 hex nibbles; a board is 61 tiles. One
// nibble per tile in centre-out spiral order (nibble 0 = the centre), 3 meta
// nibbles reserved. The nibble picks from a weighted table — [6,4,3,2,1]/16.
export const NIBBLE_TYPES = [
...
// ── the sim instance ─────────────────────────────────
export function createSim({ angle = SEED_ANGLE, pubkey = null } = {}) {
  ...
  // The world's identity: a 64-hex-nibble id (nostr pubkey — or later any
  // 32-byte id). v0 derivation: the key inscribes the HOME board's tiles in
  // reading order. Lazy and pure — nothing stored, replay-safe.
  if (pubkey != null && !/^[0-9a-f]{64}$/.test(pubkey)) throw new Error("pubkey must be 64 lowercase hex chars")
  const homeTypes = new Map()
  const homeChars = new Map() // the raw hex character(s) per home tile — the inscription itself
  if (pubkey) {
    // READING order, like text: rows top to bottom, left to right. 64 chars,
    // 61 tiles: the CENTRE holds the key's middle four; the other 60 tiles
    // take the remaining chars in reading order.
    const mid = pubkey.length / 2
    const centreChars = pubkey.slice(mid - 2, mid + 2)
    const rest = pubkey.slice(0, mid - 2) + pubkey.slice(mid + 2)
    let i = 0
    for (const t of readingOrder(RINGS)) {
      const ch = t[0] === 0 && t[1] === 0 ? centreChars : rest[i++]
      homeChars.set(key(t), ch)
      homeTypes.set(key(t), NIBBLE_TYPES[parseInt(ch[0], 16)])
    }
  }
  ...
  // home inscription, then the kind's default. typeOf feeds costs; the
  // renderer reads the name for the land's look.
  const typeNameAt = g => {
    const b = boardOf(g)
    if (b) {
      const stored = b.node && b.node.types[key(b.local)]
      if (stored) return stored
      if (b.c[0] === 0 && b.c[1] === 0) return homeTypes.get(key(b.local)) || "plain"
      return "plain"
    }
  ...
  const serialize = () => ({
    app: "anon&mato",
    schema: SCHEMA,
    world: { angle, pubkey, rings: RINGS, rules: RULES },
