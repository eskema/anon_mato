...
// ── tile types ───────────────────────────────────────
// Every hex can carry a type (sparse, per world tile: tile.types["q,r"]);
// absent = plain. A type's properties are cost MULTIPLIERS on the level base —
// all 1 for now, but this is the hook for terrain/specials with real costs.
export const TILE_TYPES = {
  plain: { move: 1, scout: 1 }
}
const typeAt = (tile, k) => TILE_TYPES[(tile && tile.types && tile.types[k]) || "plain"]
...
// The perimeter row: ring RINGS+1 around the interior. In the true super
// lattice these hexes ARE the neighbouring siblings' border tiles (each one is
// owned by exactly one sibling, at distance exactly RINGS from its centre) —
// so crossing to a neighbour is just stepping onto one of them.
export const PERIM = RINGS + 1
...
export function createSim() {
  ...
  // ── the perimeter row (the siblings' border tiles, seen from here) ──
  ...
  // A perimeter hex exists (is scoutable/crossable) when its lobe isn't sealed
  // by a wall and its sibling actually sits on the parent grid. No crossing at
  // the base — the parent view above it is still locked.
  function isPerimeter(h) {
    if (depth() <= BASE_DEPTH || Hex.length(h) !== PERIM) return false
    const i = lobeOf(h)
    if (i < 0 || walled(i)) return false
    const t = exitTarget(i)
    return inBounds(t[0], t[1])
  }

  // ── routing ──────────────────────────────────────────
  // Discovery of a perimeter hex lives where the tile lives: in the sibling.
  const isDiscovered = h => {
    if (Hex.length(h) > RINGS) {
      if (!isPerimeter(h)) return false
      const i = lobeOf(h)
      const sib = siblingNodeOf(i)
      return !!sib && sib.discovered.has(key(siblingLocal(h, i)))
    }
    return view().tile.discovered.has(key(h))
  }
  // Frontier = the player's OWN undiscovered neighbours (your immediate explore
  // options) — perimeter hexes included, by plain adjacency.
  const isFrontier = h =>
    !isDiscovered(h) &&
    (Hex.length(h) > RINGS
      ? isPerimeter(h) && Hex.distance(h, view().player) === 1
      : pathNeighbors(view().player).some(n => eq(n, h)))
  ...
  // final hop onto an undiscovered goal (a frontier tile). A perimeter goal is
  // always a final hop (stepping onto it crosses to the sibling, so routes can
  // never pass THROUGH the perimeter).
  function bfsPathDisc(start, goal, vw = view()) {
    if (eq(start, goal)) return [start]
    const perimGoal = vw === view() && Hex.length(goal) > RINGS && isPerimeter(goal)
    const prev = new Map()
    const unwind = cur => {
      const path = []
      let c = cur
    ...
    if (perimGoal && Hex.distance(start, goal) === 1) return [start, goal]
    ...
    while (queue.length) {
      ...
      for (const n of pathNeighbors(cur, vw)) {
        ...
        if (!perimGoal && eq(n, goal)) return [...unwind(cur), n]
        if (!vw.tile.discovered.has(k)) continue // can't traverse unexplored (non-goal) tiles
        seen.add(k)
        prev.set(k, cur)
        if (perimGoal && Hex.distance(n, goal) === 1) return [...unwind(cur), n, goal]
        queue.push(n)
      }
    }
    return null
  ...
  // tile type (perimeter hexes read their type from the sibling that owns them).
  ...
  const typeOf = h => {
    if (Hex.length(h) > RINGS) {
      const i = lobeOf(h)
      return typeAt(siblingNodeOf(i), key(siblingLocal(h, i)))
    }
    return typeAt(view().tile, key(h))
  }
  ...
  // discovered tile adjacent to a DISCOVERED perimeter hex (a crossing you can
  // actually take right now), or the entry itself at the home base.
  function stepsToExit(pos) {
    ...
    for (const k of v.tile.discovered) {
      ...
      for (const d of DIRS) {
        ...
        if (isPerimeter(n) && isDiscovered(n)) {
          isExit = true
          break
        }
      }
  ...
  // Reserve needed to get home AFTER crossing into sibling i: the updated
  // parent trail, the climb back out, plus every level below — the landing
  // tile itself sits right at the return crossing (0 local steps).
  ...
  function canMove(target) {
    ...
    if (Hex.length(target) > RINGS) {
      // crossing: the walk there plus the way home from the OTHER side
      const cost = view().tile.safe ? stepCostAt(target) : pathCost(path)
      return cost + crossReturn(lobeOf(target)) <= energy
    }
    return view().tile.safe || pathCost(path) + returnFrom(target) <= energy
  }

  const canScout = target =>
    isFrontier(target) && (view().tile.safe || scoutCostAt(target) + returnCost() <= energy)
  ...
  // perimeter hexes included (the crossing options show like any frontier).
  function reachableDots() {
    ...
    for (const d of DIRS) {
      ...
      if (!inBounds(n[0], n[1]) && isPerimeter(n) && !isDiscovered(n) && canScout(n)) dots.add(key(n))
    }
    return dots
  }
  ...
  // Scout: reveal an adjacent undiscovered tile without moving. A perimeter
  // hex is revealed in the sibling that owns it — same action, same rates.
  function doScout(target) {
    const v = view()
    if (!v.tile.safe) energy -= scoutCostAt(target) // discovery is free inside the safe space
    if (Hex.length(target) > RINGS) {
  ...
  // CROSS to the sibling that owns perimeter hex `h`: one ordinary step that
  // lands on that exact tile on the other side — the boards just switch. At
  // the parent scale this IS a step onto the sibling's parent tile, so the
  // parent trail extends/retraces and the tile becomes discovered up top —
  // but the cost is the plain local step (uniform for now; tile types are the
  // knob for making crossings, or any hex, pricier later).
  function doCross(h) {
    const i = lobeOf(h)
    const t = exitTarget(i)
    const parent = parentOf()
    energy -= stepCostAt(h) // charged even from the safe space — the step leaves it
  ...
  const ACTIONS = {
    move: {
      ...
      run: a => {
        ...
        const crossing = Hex.length(path[path.length - 1]) > RINGS
