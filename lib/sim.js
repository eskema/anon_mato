...
export const RULES = 5 // replay-rules version (5: home costs — steps/scouts inside safe boards charge 1) — bump on ANY change that alters what an old
...
export const STAT_NAMES = ["scout", "travel", "gather", "build", "craft", "trade", "tend", "lore"]

export function statsOf(hex64) {
  const out = {}
  for (let i = 0; i < STAT_NAMES.length; i++) {
    let s = 0
    for (let j = 0; j < 8; j++) s += parseInt(hex64[i * 8 + j], 16)
    out[STAT_NAMES[i]] = Math.round(s / 8)
  }
  return out
}
...
export const LESSON_COST = 10 // minutes per lesson — lessons spend your day
export const baseLevel = innate => Math.floor(innate / 2)
// talent speeds learning: xp per lesson, 10 (innate 0) … ~19 (innate 15)
export const lessonXp = innate => LESSON_COST * (1 + innate / 16)
...
export function createSim({ angle = SEED_ANGLE, pubkey = null, worldKey = null } = {}) {
  ...
  const skillOf = skill =>
    Math.min(SKILL_CAP, baseLevel(innateOf(skill)) + Math.floor((learned[skill] || 0) / XP_PER_LEVEL))
  // v1: an NPC's current level is half its nature (teaching raises it later,
  // never past its nature — the cap that keeps experts permanently valuable)
  const npcSkill = (npc, skill) => baseLevel(npc.stats[skill])
  ...
  const scoutCostAt = g => scoutCost() * typeOf(g).scout * (1 - skillOf("scout") / 30)
  ...
  // EXHAUSTED: out of exploring moves. From where we stand, no move to a tile
  // that ISN'T strictly closer to home is still affordable (even one spare minute
  // that can't fund a step counts as exhausted). Scouting/learning don't count —
  // they don't take us off the path. Never exhausted inside the safe home.
  function exhausted() {
    const p = view().player
    if (freeAt(p)) return false
    const rc = returnFrom(p)
    for (const n of moveNeighbors(p)) {
      if (!isDiscovered(n)) continue
      if (returnFrom(n) < rc) continue // strictly toward home — that's the way back, not exploring
      if (stepChargeAt(n) + returnFrom(n) <= energy) return false // an exploring step is still affordable
    }
    return true
  }

  // is `target` on the current way home? (used to lock movement while exhausted).
  // Home now costs, so even inside home only the tiles on the route to the rest
  // spot count — no free-roam plateau.
  const onHomePath = target => {
    const hp = homePath()
    return !!hp && hp.some(t => eq(t, target))
  }
  ...
  function canMove(target) {
    ...
    if (!e) return false
    if (e.charge + returnFrom(target) > energy) return false
    if (exhausted() && !onHomePath(target)) return false // exhausted: movement is locked to the way home
    return true
  }
  ...
  function viaValid(via, target) {
    ...
    if (exhausted() && !onHomePath(target)) return false // exhausted: only the way home
    return true
  }
  ...
  const ACTIONS = {
    move: {
      // when exhausted, canMove/viaValid already lock movement to the way home
    ...
    learn: {
      ...
      run: a => {
        const npc = teacherNear()
        energy -= LESSON_COST
        const gained = (learned[a.skill] || 0) + lessonXp(innateOf(a.skill))
        // clamp: xp never carries you past the teacher's level (or the cap)
        const ceiling = Math.min(npcSkill(npc, a.skill), SKILL_CAP)
        const maxXp = (ceiling - baseLevel(innateOf(a.skill))) * XP_PER_LEVEL
        learned[a.skill] = Math.min(gained, Math.max(0, maxXp))
      }
    },
  ...
  const LOGGED = new Set(["move", "scout", "learn", "enter", "clearBoard", "rest", "goHome", "restResume"])
  ...
  return {
    ...
    landAt: g => {
      if (!hasTerrain || !boardOf(g)) return null
      const biome = typeNameAt(g)
      const t = TILE_TYPES[biome] || TILE_TYPES.plain
      return { biome, elevation: Math.round(combinedAt(g)), move: t.move, scout: t.scout, yields: BIOME_YIELD[biome] || "—" }
    },
    npcAt,
