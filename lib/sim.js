...
export function createSim() {
  ...
  // Classify ANY frame-local coordinate, however far — the seam view uses this
  // to draw every discovered tile in the viewport, unbounded by the frame's
  // rings. Resolves through GLOBAL coords: find the board (parent hex) owning
  // the position, or the seam between boards.
  function probe(h) {
    if (depth() <= BASE_DEPTH) return null
    const tbl = SUPER_TO_PARENT_DIR[parity()]
    const b0 = SUPER[tbl.indexOf(0)] // parent DIR {1,0} at child scale
    const b1 = SUPER[tbl.indexOf(5)] // parent DIR {0,1} at child scale
    const P = parentOf().player
    const g = [P[0] * b0[0] + P[1] * b1[0] + h[0], P[0] * b0[1] + P[1] * b1[1] + h[1]]
    // invert the board lattice to find candidate parent hexes near g
    ...
    for (const c of [pc, ...Hex.neighbors(pc)]) {
      ...
      const cg = [c[0] * b0[0] + c[1] * b1[0], c[0] * b0[1] + c[1] * b1[1]]
      const d = Hex.distance(g, cg)
      if (d <= RINGS) {
        const node = parentOf().tile.children[key(c)]
        return { kind: "in", discovered: !!node && node.discovered.has(key([g[0] - cg[0], g[1] - cg[1]])) }
      }
      if (d === SEAM_RING) seams++
    }
    if (seams >= 2) return { kind: "seam", discovered: parentOf().tile.seamDiscovered.has(key(g)) }
    return null
  }
  ...
  // Shared re-framing tail for crossings and seam slides: the final step onto
  // hOld is ELASTIC like any other (retracing pops the trail instead of
  // appending — this is what keeps retraces over seams honest), then the whole
  // trail translates into board i's frame, keeping the longest suffix still
  // within the view. Callers must have updated parent.player already.
  function reframeOnto(i, hOld, t) {
    const top = view()
    if (top.trail.length >= 2 && eq(hOld, top.trail[top.trail.length - 2])) top.trail.pop()
    else top.trail.push(hOld)
    const carried = []
    ...
    const local = siblingLocal(hOld, i)
    top.tile = childAt(parentOf().tile, key(t))
    top.key = key(t)
    top.origin = t.slice()
    top.entry = local.slice()
    top.player = local.slice()
    top.trail = carried // ends with the landing tile — no duplicate appends
  }
  ...
  function doCross(h) {
    ...
    parent.player = t
    reframeOnto(i, h, t)
    journalDiscover(view().tile, key(view().player))
    markReachedEdges()
  }
