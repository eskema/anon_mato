...
export function createSim({ angle = SEED_ANGLE, pubkey = null } = {}) {
  ...
  // 32-byte id). v0 derivation: nibbles 0..60 inscribe the HOME board's tile
  // types in spiral order. Lazy and pure — nothing stored, replay-safe.
  const homeTypes = new Map()
  const homeChars = new Map() // the raw hex character per home tile — the inscription itself
  if (pubkey) {
    const spiral = spiralOrder(RINGS)
    for (let i = 0; i < spiral.length; i++) {
      homeChars.set(key(spiral[i]), pubkey[i])
      homeTypes.set(key(spiral[i]), NIBBLE_TYPES[parseInt(pubkey[i], 16)])
    }
  }

  // The hex character inscribed on a (home-board) tile, or null — the
  // renderer shows the key literally; everything else derives from it.
  const nibbleAt = g => {
    const b = boardOf(g)
