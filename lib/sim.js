...
export function createSim({ angle = SEED_ANGLE, pubkey = null, worldKey = null } = {}) {
  ...
  const retAfterPath = path => {
    ...
    if (!navWater(end)) return returnFrom(end)
  ...
  function canMove(target) {
    ...
    // bank you'd have come from
    const back = navWater(target) ? (e.on ? riverReserve(target) : returnVia(target, e.prevAt)) : returnFrom(target)
