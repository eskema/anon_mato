...
export const RULES = 4 // replay-rules version (4: skills — scout level discounts scouting) — bump on ANY change that alters what an old
...
export function createSim({ angle = SEED_ANGLE, pubkey = null, worldKey = null } = {}) {
  ...
  function exhausted() {
    ...
    for (const n of moveNeighbors(p)) {
      ...
      const reserve = freeAt(n) ? 0 : returnFrom(n)
      if (stepChargeAt(n) + reserve <= energy) return false // an exploring step is still affordable
    }
    return true
  }

  // is `target` on the current way home? (used to lock movement while exhausted).
  // Any safe-home tile counts — once inside, movement is free to the rest spot.
  const onHomePath = target => {
    if (freeAt(target)) return true
    const hp = homePath()
  ...
  function canMove(target) {
    ...
    if (e.charge + reserve > energy) return false
  ...
  function viaValid(via, target) {
    ...
    const reserve = freeAt(target) ? 0 : returnFrom(target)
    if (pathCharge(via) + reserve > energy) return false
