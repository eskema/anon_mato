...
export function createSim() {
  ...
  const canEnter = () => view().parked < 0 && depth() < MAX_DEPTH && !eq(view().player, view().entry)
  ...
  // Arriving at the centre special tile of the safe space = coming home:
  // rest + next day. (Suppressed during replay — the banked day already ended.)
  function checkHome() {
    const v = view()
    if (replaying) return
  ...
  const ACTIONS = {
    move: {
      can: a => canMove(a.target),
      run: a => {
        const path = routeTo(a.target)
    ...
    restResume: { can: () => true, run: () => restResumeRun() }
  }
