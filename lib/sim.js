...
export const RULES = 26 // replay-rules version (26: only NOVEL ground trains travel — a step practices `move` solely when it wears a tile for the first time; commutes, home and seams teach nothing) — bump on ANY change that alters what an old
...
export const PRACTICE_SKILL = { move: "travel", scout: "scout", gather: "gather", build: "build" } // action kind → the skill it trains (crafting is an NPC service, not self-taught).
...
// LEARNING BY EDGES (2026-07-16): a skill at level L is an (L+1)-sided shape;
// climbing L→L+1 fills L+1 edges, so the ladder steepens with height (≈L²/2
// edges to reach L — quick at first, an age near the cap). BOTH sources fill
// the SAME edges: a LESSON completes one whole edge, PRACTICE (doing the skill)
// trickles fractions. Fractions carry — a lesson onto a half-done edge finishes
// it and half-fills the next. Level 0 needs a single edge, so the first lesson
// always lands.
export const edgesForLevel = L => L + 1 // edges to advance from level L to L+1
...
export const RESOURCES = {
  plants: { weight: 1, min: 3, regrow: 120, shelf: 4320 }, // 3 days
  fish: { weight: 2, min: 5, regrow: 1440, shelf: 1440 }, // 1 day — spoils fast
  eggs: { weight: 1, min: 4, regrow: 4320, shelf: 5760 }, // 4 days
...
export const NODE_DENSITY = { plants: 0.55, fish: 0.5, eggs: 0.35, wood: 0.3, rock: 0.18, metal: 0.09 }
...
export function createSim({ angle = SEED_ANGLE, pubkey = null, worldKey = null } = {}) {
  ...
  let stash = {} // home-tile local key → { item, arr:[{at,uses?}] } — the identity tiles double as storage cells (log-derived)
  ...
  const blocked = g => !isCentre(g) && !!typeOf(g).impassable
  ...
  const deepnessAt = g => Math.max(0, Math.min(WATER_LEVEL, WATER_LEVEL - Math.round(smoothedAt(g))))
  ...
  const worldMin = () => (day - 1) * 1440 + ((dayStart ? dayStart.energy : SEED_MIN) - energy)
  ...
  const scoutCostAt = g => scoutCost() * (isCentre(g) ? 1 : typeOf(g).scout) * (1 - skillOf("scout") / 30)
  ...
  // prices the walk to the home centre from inside home too.
  const returnCost = () => returnFrom(view().player)
  ...
  function homePathFrom(src) {
    ...
    if (!prev.has(key(home))) return null
    const path = []
    for (let cur = home; cur; cur = prev.get(key(cur))) {
      path.unshift(cur)
      if (eq(cur, src)) break
    }
  ...
  function canMove(target) {
    ...
    if (!e) return false
    return e.charge + returnFrom(target) <= energy
  }
  ...
  function viaValid(via, target) {
    ...
    if (pathCharge(via) + returnFrom(target) > energy) return false
    return true
  }
  ...
  // the STASH cell for the tile underfoot: a HOME-board tile (not the centre)
  // doubles as a one-item storage device, keyed by its board-local coord.
  // null anywhere else — you can drop outside, but it's gone for good.
  const stashKeyAt = g => {
    const b = boardOf(g)
    if (!b || b.c[0] !== 0 || b.c[1] !== 0) return null // home board only
    if (b.local[0] === 0 && b.local[1] === 0) return null // not the centre
    return key(b.local)
  }
  ...
  const ACTIONS = {
    ...
    // DROP one item off your back onto the tile underfoot. On a HOME tile it
    // STASHES (recoverable by `take` — one item type per tile); anywhere
    // else it's discarded for good. Free and instant.
    drop: {
      can: a => {
        if (!a.item || countOf(a.item) < 1) return false
        const sk = stashKeyAt(view().player)
        if (sk && stash[sk] && stash[sk].item !== a.item) return false // one type per cell
        return true
      },
      run: a => {
        ...
        worldStamp++ // pack (and maybe a stash) changed
      }
    },
    ...
    rest: {
      ...
      can: () => restSpots.some(s => eq(s, view().player)),
      run: () => doRest()
    },
    ...
    restResume: {
      ...
      can: () => dailyBudget() - returnCost() >= returnCost(),
      run: () => restResumeRun()
    }
  }
  ...
  return {
    ...
    root: () => stack[0].tile,
    energy: () => energy,
    dayBudget: () => (dayStart ? dayStart.energy : SEED_MIN), // the full window this day began with
    ...
    returnFrom,
    homePath,
    homePathFrom: src => homePathFrom(src), // the way home from a given tile (the ghost mid-move)
