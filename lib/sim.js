...
// The seam tile where the seed angle's ray exits the home board: walk the ray
// outward (angle convention from the setup picker: 0° points up, clockwise)
// until it lands on the seam ring. Pure — the gate falls on the EXACT tile
// the angle points through, side seam or post alike.
export function gateTileFor(angleDeg, parity = 0) {
  ...
  for (let t = 0.5; t < SEAM_RING * 3; t += 0.05) {
    ...
    if (Hex.length(h) === SEAM_RING) return h
    if (Hex.length(h) > SEAM_RING) break
  }
  return [SEAM_RING, 0] // unreachable fallback
}
...
// The home's gate: the single seam tile the seed angle exits through. The
// parent-scale gate direction derives from it (the lobe that seam tile faces) —
// used only by the locked base view's centre↔gate link and its visuals.
export const GATE_TILE = gateTileFor(SEED_ANGLE)
...
export function createSim() {
  ...
  function journalDiscover(tile, k) {
    ...
    // a walled board's gate opens once the whole board is discovered — a
    // ratchet, like discovery itself (it never closes again)
    if (tile.walls && tile.gate && !tile.gateOpen && tile.discovered.size >= BOARD_TILES) tile.gateOpen = true
  }
  ...
  // a walled board is sealed along its WHOLE seam ring except its gate tile —
  // and the gate only counts once it has opened.
  function wallBlocks(s) {
    const t = view().tile
    if (!t.walls || Hex.length(s) !== SEAM_RING) return false
    return !(t.gateOpen && t.gate === seamKey(s))
  }

  // The crossing wall check: entering board i across seam hex s is blocked by
  // THEIR walls unless s is their (open) gate — the global seam key reads the
  // same from both sides, so the comparison is frame-independent.
  function nbrWallBlocks(s, i) {
    const sib = siblingNodeOf(i)
    if (!sib || !sib.walls) return false
    return !(sib.gateOpen && sib.gate === seamKey(s))
  }
  ...
  function playerExits() {
    ...
    for (const n of walkNeighbors(view().player)) {
      ...
      if (lobes.length === 1 && !wallBlocks(n)) set.add(lobes[0])
    }
    return set
  }
  ...
  function doEnterHome() {
    ...
    child.safe = true
    child.walls = true // sealed along the whole seam ring…
    stack.push(
    ...
    // …except the gate: the seam tile the seed angle exits through (stored as
    // a global seam key, so both sides of the edge agree). Starts CLOSED —
    // journalDiscover opens it once the whole board is discovered.
    if (!child.gate) child.gate = seamKey(GATE_TILE)
  }
