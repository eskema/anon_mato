...
export const RULES = 27 // replay-rules version (27: the shape IS the level — climbing L→L+1 fills L edges (max 1), so every polygon wears exactly its level's sides; the whole ladder shifts one edge cheaper) — bump on ANY change that alters what an old
