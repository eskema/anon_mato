...
export const LEAP = true // the leapfrog power move: jump 2 tiles straight through an edge for ONE
// step's price (the landing tile's). Dev-on for playtesting; later an unlockable ability.
...
export function createSim() {
  ...
  function boardOf(g) {
    ...
    for (const c of [pc, ...Hex.neighbors(pc)]) {
      ...
      if (Hex.distance(g, centre) <= RINGS) {
        return { c, centre, local: [g[0] - centre[0], g[1] - centre[1]], node: parentOf().tile.children[key(c)] }
      }
    }
    return null
  }
  ...
  function isSeamAt(g) {
    ...
    for (const c of [pc, ...Hex.neighbors(pc)]) {
      ...
      if (Hex.distance(g, boardCentre(c)) === SEAM_RING) seams++
    }
    return seams >= 2
  }
  ...
  function kindOf(g) {
    ...
    if (boardOf(g)) return "in"
    return isSeamAt(g) ? "seam" : null
  }
  ...
  // Leap targets: two rows out, straight through the shared edge (g + 2·dir).
  // Legal when the jumped-over middle is discovered walkable ground and
  // neither crossed edge is walled. The middle is jumped OVER — never stood
  // on, never charged; the leap prices as ONE step onto the landing, so
  // routing prefers it wherever the ground is known. Chains naturally: each
  // leap is one edge of the move graph.
  function leapNeighbors(g) {
    if (!LEAP) return []
    const out = []
    for (let d = 0; d < 6; d++) {
      const m = [g[0] + DIRS[d].q, g[1] + DIRS[d].r]
      if (!kindOf(m) || !isDiscovered(m)) continue // you leap over KNOWN ground
      const t = [m[0] + DIRS[d].q, m[1] + DIRS[d].r]
      if (!kindOf(t)) continue
      if (stepBlocked(g, m, d) || stepBlocked(m, t, d)) continue
      out.push(t)
    }
    return out
  }
  ...
  // Frontier = the player's OWN undiscovered walkable neighbours (scouting
  // stays adjacent-only — no leap-scouting).
  const isFrontier = g => !isDiscovered(g) && !!kindOf(g) && walkNeighbors(view().player).some(n => eq(n, g))

  // Shortest route start→goal over DISCOVERED ground, with a single allowed
  // final hop onto an undiscovered goal. One graph — boards and seam alike.
  function bfsPathDisc(start, goal) {
    ...
    while (queue.length) {
      ...
      for (const n of moveNeighbors(cur)) {
        ...
        if (!isDiscovered(n)) continue // can't traverse unexplored (non-goal) tiles
        seen.add(k)
        prev.set(k, cur)
        queue.push(n)
      }
  ...
  const typeOf = g => {
    const b = boardOf(g)
    if (b) return TILE_TYPES[(b.node && b.node.types[key(b.local)]) || "plain"]
    return TILE_TYPES[parentOf().tile.seamTypes[key(g)] || "seam"]
  }
  ...
  // ── the reserve (the EXACT way home) ────────────────
  // The reserve is the true cheapest cost of walking from a position back to
  // the day's entry over discovered ground — computed as one Dijkstra sweep
  // from the entry (edge weight = the charge of the tile stepped onto, going
  // homeward) and cached until the world or the entry changes. This is what
  // makes never-strandable LITERAL: at energy == reserve the trip home is
  // affordable to the minute, every minute.
  let worldStamp = 0 // bumped on discovery/wall changes — invalidates the map
  let reserveCache = { stamp: -1, entry: "", map: null }

  function reserveMap() {
    const entry = view().entry
    const ek = key(entry)
    if (reserveCache.stamp === worldStamp && reserveCache.entry === ek) return reserveCache.map
    const dist = new Map([[ek, 0]])
    ...
    reserveCache = { stamp: worldStamp, entry: ek, map: dist }
    return dist
  }
  ...
  const LOGGED = new Set(["move", "scout", "enter", "clearBoard"])
