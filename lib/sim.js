...
export function createSim() {
  ...
  // Step distances over the discovered walk graph (frontier hexes get a
  // distance but aren't expanded past).
  ...
  const typeOf = g => {
    ...
    return TILE_TYPES[parentOf().tile.seamTypes[key(g)] || "plain"]
  }
  ...
  // ── the reserve (the honest-enough way home) ────────
  // Shortest hop count from `pos` to the nearest discovered seam hex (the way
  // out of whatever board you're on; 0 when already on discovered seam).
  function stepsToExit(pos) {
    ...
    for (const [k, d] of dist) {
      ...
      if (kindOf(h) === "seam" && isDiscovered(h)) best = d
    }
    if (best < Infinity) return best
    return 0
  }

  // Parent-scale legs of the way home after walking `path` (or from the
  // current state when no path): the committed parent trail, elastically
  // replayed over every board boundary the path crosses.
  function parentLegsAfter(path) {
    const parent = parentOf()
    const legs = parent.trail.map(t => key(t))
    let cur = key(parent.player)
    if (path) {
      for (const g of path) {
        const c = boardHexOf(g)
        if (!c || key(c) === cur) continue
        const ck = key(c)
        if (legs.length >= 2 && legs[legs.length - 2] === ck) legs.pop()
        else legs.push(ck)
        cur = ck
      }
    }
  ...
  // Shortest time home from `pos` (after walking `path`, when given): local
  // steps to the nearest exit + the parent trail's legs at the parent rate +
  // a parent-scale move per climb-out. An approximation, recomputed live.
  function returnFrom(pos, path = null) {
    let c = stepsToExit(pos) * (view().cost * MOVE_FRACTION)
    for (let i = 0; i < stack.length - 1; i++) {
      const lv = stack[i]
      const legs = i === stack.length - 2 ? parentLegsAfter(path) : lv.trail.length - 1
      c += legs * (lv.cost * MOVE_FRACTION)
      if (i + 1 > BASE_DEPTH) c += lv.cost * MOVE_FRACTION // the climb back out of level i+1
    }
    return c
  }
  ...
  function canMove(target) {
    ...
    const reserve = freeAt(target) ? 0 : returnFrom(target, path)
  ...
  function viaValid(via, target) {
    ...
    const reserve = freeAt(target) ? 0 : returnFrom(target, via)
