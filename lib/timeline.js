// The clock across the top — stacked scales driven by the game (energy = minutes;
// time spent = minutes past 06:00). Scales: YEAR (bars = years earned, 1 for now) /
// MONTH (12) / DAY-of-month (30) / HOUR (24) / MINUTE (60).
//
// Collapsed (default): scales are thin hairlines, MINUTE is tall; the scale LABELS
// live merged on the status line (drawn by grid.js). Expanded: each scale gets its
// own label line above a tall bar. The remaining budget ("available") is a
// left-aligned row of taller/thicker bars, independent of the minute slots.
// The reserved return trip is shown implicitly (the medium segment on the bars).
...
const TOP = 28
const PAD_L = 12
const PAD_R = PAD_L // symmetric margins now that the play button moved onto the status line
const SCALE_H = 2 // collapsed thin scale height (the "line thickness")
const GAP = 1
...
function buildRows(clock) {
  ...
  return [
    { label: `year ${Math.floor(dayIdx / (MONTHS * DAYS_PER_MONTH)) + 1}`, steps: years, cur: Math.floor(dayIdx / (MONTHS * DAYS_PER_MONTH)) },
    { label: `month ${(Math.floor(dayIdx / DAYS_PER_MONTH) % MONTHS) + 1}`, steps: MONTHS, cur: Math.floor(dayIdx / DAYS_PER_MONTH) % MONTHS },
    { label: `day ${day}`, steps: DAYS_PER_MONTH, cur: dayIdx % DAYS_PER_MONTH },
    { label: `hour ${6 + Math.floor(used / 60)}`, steps: 24, cur: Math.floor(totalUsed / 60), com: Math.floor(totalSpent / 60), hour: true },
    { label: `minute ${Math.round(used % 60)}`, steps: 60, cur: Math.floor(used) % 60, com: Math.min(60, Math.ceil(spent)), minutes: true }
  ]
}
...
function drawBars(ctx, ink, row, x, y, w, h) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (asleep) {
      ...
      ctx.fillStyle = ink
      ctx.globalAlpha = i === row.cur ? 0.9 : i < row.cur ? 0.25 : i < com ? 0.45 : 0.12
      ctx.fillRect(bx, y, inner, h)
    }
  }
  ctx.globalAlpha = 1
...
function fmtBudget(free) {
  // remaining budget as "3d 12h 34m until rest" — only the non-zero parts. Minutes
  // stay as minutes until they exceed 60 (60 → "60m until rest", 61 → "1h 1m …").
...
function drawAvailable(ctx, ink, x, y, free, font, at, action) {
  // position · budget on one line; a hovered/committed action cost trails after it —
  // lighter while just hovering, darker once committed (until the action completes).
  ctx.font = font
  ctx.textAlign = "left"
  ...
  if (action?.text) {
    ctx.globalAlpha = action.committed ? 0.9 : 0.25
    ctx.fillText("  " + action.text, x + ctx.measureText(base).width, y)
  }
  ctx.globalAlpha = 1
}

// Right-aligned text buttons at the end of the budget line (e.g. go home / rest & resume).
// Records hit-boxes into `out` for the grid to test.
...
export function createTimeline() {
  ...
  // Interaction is retired — the clock follows the game. These stubs keep the old
  // call sites in grid.js harmless (they resolve falsy/null and fall through).
  const no = () => false
  const nul = () => null
  return {
    draw,
    playButton,
    homeButtons: () => homeBoxes,
    height,
    advance: () => {},
    toggle: () => {},
    setHover: () => false,
