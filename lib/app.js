// App engine.
//
// Owns the canvas and the layout (recomputed on resize). It shows one screen
// at a time and forwards pointer/render to it. It is flow-agnostic: callers
// pick the starting screen via setScreen().
...
export function createApp(canvas) {
  ...
  function backdrop() {
    const ch = [[], [], []]
    try {
      // NEVER getImageData the MAIN canvas (2026-08-31): a single readback can
      // demote it to CPU rendering for the rest of the session — the whole
      // game turns laggy (it did). The frame is squeezed onto a tiny helper
      // canvas instead (a GPU-side drawImage), and only the 9×9 helper —
      // marked willReadFrequently — is ever read back: one call, 81 cell
      // means, and the MEDIAN of those still lands on the ground colour.
      const S = 9
      ...
      for (let p = 0; p < d.length; p += 4) for (let k = 0; k < 3; k++) ch[k].push(d[p + k])
    } catch {
