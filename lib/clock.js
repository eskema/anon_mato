...
export function drawMoon(ctx, m, ink, mr = 6, surface = null) {
  const { x, y } = m.pos
  ctx.globalAlpha = 1
  // A NEW MOON IS ONE BLACK DOT: the halo AND the outline ride the moon's own
  // light — nothing to lift off the map and nothing to rim when no sun reaches
  // it. `lit` is full by the quarter, so every readable phase keeps its halo and
  // crisp outline; only the last sliver goes dark.
  const lit = Math.min(1, m.illum * 2)
  if (surface && lit > 0.01) {
    ...
    g.addColorStop(0, rgbaOf(surface, 0.55 * lit))
  ...
  // the lit side — properly LUMINOUS (2026-08-31: at 86% it read dim once the
  // halo and the phase shadow went night-dark; moonlight should glow)
  ctx.fillStyle = "hsl(220 25% 95%)"
  ctx.beginPath()
  ...
  ctx.fill()
  ctx.restore()
  ctx.strokeStyle = ink // the outline — full --text through every lit phase…
  ctx.globalAlpha = lit // …and gone at new, where the moon has no edge to show
  ctx.lineWidth = 1
  ctx.beginPath()
  ctx.arc(x, y, mr, 0, TAU)
  ctx.stroke()
