...
export function HexGridScreen() {
  ...
  function onPointerMove(p) {
    ...
    if (h && !eq(h, v.player)) {
      ...
      // along the trail or nothing (never a shortest-route fallback) — except
      // trail carried across the seam, where walking back IS a crossing
      if (sim.trailIndexOf(h) >= 0) {
        np = sim.retraceRoute(h)
        if (!np && sim.kindOf(h) === "nbr" && sim.canMove(h)) np = sim.routeTo(h)
  ...
  function onPointerDown(p) {
    ...
    if (sim.trailIndexOf(t) >= 0) {
      ...
      // retrace isn't affordable the click does nothing, never a shortest-route
      // walk. Trail carried across the seam is the exception: going back there
      // is a crossing, an ordinary move.
      const via = sim.retraceRoute(t)
      if (via) startMove(t, { via })
      else if (sim.kindOf(t) === "nbr" && sim.canMove(t)) startMove(t)
      return
    }
