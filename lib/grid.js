...
// The world a stored save was played in ({angle, pubkey, day}), or null when
// there's no (readable) save — boot and the identity card read this.
export function savedWorld() {
  try {
    ...
    return w && typeof w.angle === "number"
      ? { angle: w.angle, pubkey: w.pubkey ?? null, day: save.today?.day ?? 1 }
      : null
  } catch {
    return null
  }
