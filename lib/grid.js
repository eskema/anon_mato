...
export function HexGridScreen({ angle, pubkey = null, worldKey = null, sim: preSim = null, onReset } = {}) {
  ...
  function infoCard() {
    ...
    // the PACK rides every card: what's on your back, how heavy, and the
    // state of it — perishables flagged when spoiling soon, tools by wear
    const packRows = () => {
      const detail = sim.packDetail()
      const items = detail
        .map(d => {
          let s = `${d.k} ${d.n}`
          if (d.uses != null) s += ` (${d.uses} uses)`
          else if (d.spoilsIn != null && d.spoilsIn <= 60) s += " · spoiling"
          return s
        })
        .join(", ")
      const rows = [
        ["load", `${Math.round(sim.loadOf() * 10) / 10}/${sim.carryCap()}`],
        ["pack", items || "—"]
      ]
      const st = sim.stashHere()
      if (st) rows.push(["stashed", `${st.item} ${st.n}`])
      return rows
    }
    // anywhere on the safe home board reads as "home" (its tiles aren't land)
    if (v.tile.safe) return { kind: "info", title: "home", subtitle: "your board", rows: packRows() }
    const land = sim.landAt(v.player)
    if (!land) return { kind: "info", title: "you", subtitle: "the pack", rows: packRows() }
