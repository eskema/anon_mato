...
export function HexGridScreen({ angle, pubkey = null, worldKey = null, onReset } = {}) {
  ...
  function menuActions() {
    ...
    // lessons: the board's figure teaches what it still outranks you in —
    // strongest advantage first, capped to leave room on the ring
    for (const l of sim
      .learnable()
      .sort((x, y) => y.teacher - y.at - (x.teacher - x.at))
      .slice(0, 4)) {
      a.push({ label: `learn ${l.skill} ${l.at}→${l.at + 1}`, run: () => act({ type: "learn", skill: l.skill }) })
    }
    return a
  }
