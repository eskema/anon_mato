...
export function HexGridScreen() {
  ...
  function pendingView() {
    ...
    const p = pending
    const inflightMin = Math.min(Math.floor(p.elapsed / MS_PER_MIN), Math.ceil(p.totalMin))
    let ghostTile = null
    let ghostTrail = null
    if (p.path) {
  ...
  function stopReplay() {
    ...
    const log = sim.log()
    while (replayIdx < log.length) sim.apply(log[replayIdx++])
    sim.endReplay()
    replaying = false
    api.requestRender()
  }
  ...
  function onPointerMove(p) {
    ...
    if (h) {
      if (v.parked >= 0) {
        ...
        // hovering a trail tile previews the RETRACE back to it
        np = sim.retraceRoute(h) || (sim.canMove(h) ? sim.routeTo(h) : null)
      }
    }
