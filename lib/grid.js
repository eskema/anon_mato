...
export function HexGridScreen({ angle, pubkey = null, worldKey = null, sim: preSim = null, onReset } = {}) {
  ...
  function onPointerMove(p) {
    ...
    // the player cube is a button (opens the radial menu); an illegal target is
    // not clickable
    const onPlayer = h && eq(h, v.player)
    document.body.style.cursor = (hoverPath && !hoverIllegal) || onPlayer ? "pointer" : illegal ? "not-allowed" : "default"
  }
