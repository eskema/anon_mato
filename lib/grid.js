...
import { createSim, inBounds } from "./sim.js"
...
export function HexGridScreen() {
  ...
  function startMove(target, { via = null } = {}) {
    ...
    const crossing = !inBounds(target[0], target[1])
    pending = {
  ...
  function onPointerMove(p) {
    ...
    const h = inBounds(q, r) || sim.isPerimeter([q, r]) ? [q, r] : null
    const v = sim.view()
  ...
  function onPointerDown(p) {
    ...
    const { q, r } = renderer.pixelToHex(api.layout, p.x, p.y, size)
    const t = [q, r]
    if (!inBounds(q, r) && !sim.isPerimeter(t)) return // beyond the field
    const v = sim.view()
