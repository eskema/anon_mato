...
export function HexGridScreen({ angle, pubkey = null, worldKey = null, sim: preSim = null, onReset } = {}) {
  ...
  let groundHover = null // …and the tile-pile box (lower-right) the pointer is over
  ...
  let helpersOpen = false // …and the helpers list beside the TITLE. All three open
  // independently now: opening one no longer shuts the others.
  ...
  function pendingView() {
    ...
    if (p.path) {
      ...
      // report where the raft actually is RIGHT NOW. On water it rides the glide
      // itself; the moment the ghost steps ashore it moors at the last water
      // tile it crossed, which is exactly where the sim will leave it.
      if (sim.aboard()) {
        if (sim.isRiver(ghostTile)) raftPos = ghostPos
        else
          for (let i = Math.min(Math.ceil(dist), p.path.length - 1); i >= 0; i--)
            if (sim.isRiver(p.path[i])) {
              raftPos = p.path[i]
              break
            }
      }
  ...
  function menuSpec() {
    ...
    if (sim.atRestSpot())
      self.push({ id: "rest", icon: "sleep", label: "sleep", closeMenu: true, ...(atHome ? { slot: "W" } : {}), run: beginSleep })
    // PLAY sits on the player's RIGHT, opposite the bed: the two things the
    // home centre is for, one either side of you.
    if (v.tile.safe && eq(v.player, [0, 0])) {
      self.push({ id: "cube", icon: "play", label: "play", slot: "E", run: enterCube })
    }
    ...
    if (gi) {
      self.push({
        ...
        icon: "gather",
        label: gi.lacks
          ? `${gi.res} · needs a ${gi.lacks}`
          : !gi.ready
            ? "gather · regrowing"
            : gi.full
              ? "gather · pack full"
              : `gather ${gi.res} · ${Math.ceil(gi.cost)}m`,
    ...
    // THE RAFT: standing in the water, with the debris hauled here and dropped,
    // build the first vehicle. Shown on any river tile so the price is visible
    // BEFORE the haul — greyed with what it still needs (DESIGN.md, *Rivers*).
    const rp = sim.raftPlan()
    if (rp && !rp.built) {
      const short = rp.needs - rp.have
      self.push({
        id: "raft",
        icon: "build",
        label: short > 0 ? `build raft · needs ${short} debris here` : "build raft",
    ...
    // board's CENTRE has been discovered offers a trip to that centre. Cost model
    // (settled 2026-07-22): the charge of WALKING there — the shortest discovered
    // path to the centre — and it walks it too, on the same pre-solved route and
    // per-tile pacing as any move. Same reserve gate as walking (canMove).
    ...
    if (hb && hb[0] === 0 && hb[1] === 0 && !eq(v.player, [0, 0])) {
      ...
      if (node && node.discovered.has("0,0")) {
        ...
        self.push({
          ...
          // …and it TRAVELS it: the same pre-solved walk as go home, so the route
          // it charges you for is the route you watch (a jump you never see is
          // just a teleport that happens to cost like a walk)
  ...
  function onPointerMove(p) {
    ...
    if ((itHit?.k ?? null) !== itemHover) {
      itemHover = itHit?.k ?? null
      scheduleRender()
    }
    if (itHit) {
      if (hovered || hoverPath) (hovered = hoverPath = null), (hoverIllegal = false)
    ...
    if ((grHit?.k ?? null) !== groundHover) {
      groundHover = grHit?.k ?? null
      scheduleRender()
    }
    if (grHit) {
      if (hovered || hoverPath) (hovered = hoverPath = null), (hoverIllegal = false)
  ...
  function onPointerDown(p) {
    ...
    if (daysOpen) {
      ...
      if (d) {
        // the list STAYS OPEN — you came here to compare days, and closing it
        // after every pick would make that a chore
    ...
    if (logsOpen) {
      ...
      // …and the entry itself: stand where the world stood just BEFORE it played
      const r = renderer.logRowHit(p)
      if (r) {
        browseTo(browse ? browse.day : liveSim.day(), Math.max(0, r.i - 1), { dip: false })
        return
      }
    }
  ...
  function draw(ctx, L) {
    ...
    renderer.draw(ctx, L, {
      ...
      restBtn: false, // was: !dayEnding && sim.atRestSpot() && !menuOpen && sim.log().length > 0
      gatherBtn: false, // was: !dayEnding && !menuOpen && sim.canAct({ type: "gather" })
    ...
    if (renderer.camAnimating() || renderer.menuAnimating() || renderer.restAnimating()) scheduleRender()
  }
