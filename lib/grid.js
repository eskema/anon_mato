...
export function HexGridScreen({ angle, pubkey = null, worldKey = null, sim: preSim = null, onReset } = {}) {
  ...
  function onPointerDown(p) {
    ...
    // a press on the world (menu closed): don't act yet — arm it. You can press,
    // drag your finger to aim (the trail previews under it), and the tile action
    // fires on RELEASE.
    worldPress = true
  }
  ...
  function onPointerUp(p) {
    ...
    if (eq(t, v.player)) {
      // lifting on the player opens the radial action menu
      menuOpen = true
      menuOpenId = menuFocusId = null
      api.requestRender()
      return
