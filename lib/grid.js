...
export function HexGridScreen({ angle, pubkey = null, worldKey = null, sim: preSim = null, onReset } = {}) {
  ...
  function cancelMove() {
    ...
    const fits = c => {
      ...
      return isFinite(back) && sim.pathCharge(part2) + back <= sim.energy() + 1e-9
    }
    ...
    if (skill === "craft") {
      ...
      out.push({
        ...
        lines: [
          ...
          { text: `debris x${RAFT_DEBRIS}`, alpha: 0.6, small: true },
  ...
  function onPointerDown(p) {
    ...
    if (renderer.homeHit(p)) {
      ...
      if (hp && sim.pathCharge(hp) <= sim.energy()) startMove(hp[hp.length - 1], { cam: true }) // go home rides in frame
      return
    }
