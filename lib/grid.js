...
// The angle a stored save was played at, or null when there's no (readable)
// save — main.js uses this to decide setup flow vs straight into the game.
export function savedAngle() {
  try {
    const raw = localStorage.getItem(SAVE_KEY)
    const a = raw ? JSON.parse(raw)?.world?.angle : null
...
export function HexGridScreen({ angle, onReset } = {}) {
  let sim = createSim({ angle })
  if (!loadSave(sim)) sim = createSim({ angle })
  persist(sim) // bank the world (its angle) immediately — a fresh pick survives reload
