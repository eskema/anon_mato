...
export function HexGridScreen({ angle, pubkey = null, worldKey = null, onReset } = {}) {
  ...
  function menuSpec() {
    ...
    // them: the board's figure, but only when you're standing beside it
    const them = []
    let themPos = null
    const npc = facedNpc()
    if (npc) {
      ...
      for (const l of lessons) l.keepOpen = true // lessons repeat — keep the ring up
    ...
    return { self, them, themPos, openId: menuOpenId, focusId: menuFocusId }
  }

  // stats panel data for the renderer (player or faced figure)
  function statsPanel() {
    const v = sim.view()
    if (statsShow === "you" && sim.playerStats()) {
      ...
      return { title: "your skills", subtitle: "you", stats: nature, effective: eff }
    }
    if (statsShow === "them") {
      const npc = facedNpc()
      if (!npc) return null
      ...
      return { title: "figure", subtitle: place, stats: npc.stats, effective: eff }
    }
    return null
  }
  ...
  // the figure you're facing: on the current board, within a step of its
  // centre (where it stands) — not merely somewhere on the same board
  function facedNpc() {
    const v = sim.view()
    const b = sim.boardHexOf(v.player)
    const npc = b && sim.npcAt(b)
    return npc && Hex.distance(v.player, npc.pos) <= 1 ? npc : null
  }
