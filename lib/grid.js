...
// The world a stored save was played in ({angle, pubkey}), or null when
// there's no (readable) save — main.js uses this to decide setup vs game.
export function savedWorld() {
  try {
    const raw = localStorage.getItem(SAVE_KEY)
    const w = raw ? JSON.parse(raw)?.world : null
    return w && typeof w.angle === "number" ? { angle: w.angle, pubkey: w.pubkey ?? null } : null
  } catch {
    return null
  }
}

export function HexGridScreen({ angle, pubkey = null, onReset } = {}) {
  let sim = createSim({ angle, pubkey })
  if (!loadSave(sim)) sim = createSim({ angle, pubkey })
  persist(sim) // bank the world (angle + identity) immediately — a fresh pick survives reload
