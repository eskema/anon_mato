...
function classify() {
  ...
  for (const [gk, t] of world) {
    ...
    let out = b
    // a peak is the SUBKEY's own e/f on mountain ground — literal digits stay
    // special (2/16 of mountain tiles), regardless of how high the base runs
    if (b === "mountain") out = state.local.get(gk) >= 14 ? "peak" : water ? "cliff" : "mountain"
