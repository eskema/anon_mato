...
const state = {
  ...
  rawView: false,
  raw: new Map(), // gkey → 0..15 (seams get the mean of their flanks)
  smooth: new Map(),
...
async function derive() {
  ...
  const boards = [...new Set([...world.values()].filter(t => t.kind === "in").map(t => t.boardKey))]
  await Promise.all(
  ...
  for (const [gk, t] of world) {
    ...
    state.raw.set(gk, parseInt(streams.get(t.boardKey)[localIndex.get(t.localKey)], 16))
  }
  for (const [gk, t] of world) {
...
function classify() {
  ...
  state.smooth = cur
  // bases: raw spikes stay mountains, water reads the smoothed field
  const base = new Map()
  for (const [gk, t] of world) {
    if (t.kind !== "in") continue
    const raw = state.raw.get(gk)
    base.set(gk, raw >= 12 ? "mountain" : state.smooth.get(gk) < state.wl ? "water" : "plain")
  }
  ...
  for (const [gk, t] of world) {
    ...
    let out = b
    if (b === "mountain") out = state.raw.get(gk) >= 14 ? "peak" : water ? "cliff" : "mountain"
