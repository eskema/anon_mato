...
// everything some biome offers, in the RESOURCES order (forage, hunt, materials)
const RES_ORDER = Object.keys(RESOURCES).filter(r => Object.values(BIOME_YIELD).some(list => list.includes(r)))
const sourcesOf = res =>
  Object.entries(BIOME_YIELD)
    .filter(([, list]) => list.includes(res))
    .map(([b]) => b)
...
// where a recipe can be made. Most ask nothing of the ground — only a recipe
// that names a `site` pins you to a biome (and then to that land's home skill)
const siteOf = site => (site ? `on ${site} · favours ${BIOME_SKILL[site]}` : "anywhere")
...
S(
  "the loop",
  `<p class="lead">Everything runs through one day-and-reserve economy. You <b>gather</b> raw resources from forage nodes out past the seam, <b>carry</b> them (weight vs capacity), and <b>craft</b> them into gear (anywhere, unless a recipe names its ground), or <b>build</b> on the land underfoot. Home is your base and map — not a place you harvest.</p>`,
  `<div class="loop">
     ...
     <div class="arr">→</div>
     <div class="node"><b>craft</b><span>gear you carry</span></div>
     <div class="arr">→</div>
...
S(
  ...
  `<p><b>The body has two needs of its own.</b> You sleep at least <code>${SLEEP_MIN / 60}h</code> a day — that is what caps the waking window. And you can't go more than <code>${HUNGER_MAX / 60}h</code> without eating: waking sets the meal clock that far out, and the reserve prices against it exactly as it does against the budget — you can only go where you can still get back to a resting place before your next meal is due. No food on your back means a three-hour day however long the budget. <b>A meal is worth its own time:</b> a bite pushes the clock on by the food's own nourishment — a scrap of herb by three minutes, a cooked hare by forty-five — so what you carry is what your day is worth. (It still adds that nourishment to the day's window too, up to the ration cap.)</p>`
)

// resources table
S(
  "resources — what the land yields",
  `<p>Every biome feeds you, each in its own way. <b>Forage</b> is what you pick up by hand — it trains gather. A <b>hunt</b> is what you have to catch with a tool of the right kind on your back (fish want a net, hare a snare, eel a spear) — it trains hunt, pays more, and spoils within the day. The peak grows nothing: carry food up.</p>`,
  `<p class="muted">A take costs minutes (eased toward half by its skill, and by a better tool). Each unit weighs on your back. A tile's yield regrows on its own clock per resource; a raw harvest also spoils after its shelf life (stone and metal keep). Node rarity is the chance a matching biome tile is actually a node for that resource.</p>`,
  `<table><thead><tr><th>resource</th><th>from</th><th>how</th><th>weight</th><th>take</th><th>eaten</th><th>regrows in</th><th>spoils in</th><th>node rarity</th></tr></thead><tbody>${RES_ORDER.map(
    r => {
      const d = RESOURCES[r]
      ...
      const how = d.hunt ? `hunt · ${d.hunt}` : d.tool ? `by hand · an ${d.tool} eases it` : "by hand"
      return `<tr>
        <td><span class="name">${swatch(RES_COL[r] || "#b8b2a6")}${r}</span></td>
        <td><span class="row">${src}</span></td>
        <td>${how}</td>
        <td>${dots(d.weight)}</td>
        <td>${d.min}m</td>
        <td>${d.food ? `+${d.food}m` : `<span class="muted">—</span>`}</td>
        <td>${span(d.regrow)}</td>
        <td>${span(d.shelf)}</td>
...
// craft
S(
  "craft — what you make, and carry",
  `<p>A craft is something that is <b>yours</b>: it rides on your back, it can be spent, worn out or dropped, and it changes what <i>you</i> can do. (Build is the mirror — a thing that becomes the place and stays there.) You make it yourself, with your own <b>craft</b> level and your own minutes, and the making trains you.</p>`,
  `<p>You can make it <b>anywhere</b> — out on the land or at your own fire. A recipe may name a <b>site</b>, a biome that has to be underfoot, and then it can only be made there; none of the crude tier does. Products are lighter than their inputs, so a craft never breaks the reserve — and converting a heavy haul into light gear before the walk home is worth doing wherever you happen to stop.</p>`,
  `<p><b>Tools come in tiers and never wear out.</b> The crude tier is heavy and slow; the fine tier, unlocked by a higher craft level, is lighter and quicker — and it <b>replaces</b> the crude one: the upgrade takes the tool you carry as one of its materials. You carry one tool of a kind; making a second is refused.</p>`,
  `<div class="cards">${Object.entries(RECIPES)
    .map(([name, r]) => {
      ...
      if (r.tool) {
        const serves = Object.keys(RESOURCES).filter(k => RESOURCES[k].hunt === r.tool || RESOURCES[k].tool === r.tool)
        effects.push(`${RESOURCES[serves[0]]?.hunt ? "hunts" : "eases"} ${serves.join(", ")}${r.speed < 1 ? ` · ${Math.round((1 - r.speed) * 100)}% quicker` : ""}`)
        effects.push(`weighs ${r.weight}`)
      }
      return `<div class="card">
        <h3>${r.site ? swatch(BIOME_COL[r.site]) : ""}${name}</h3>
        <div class="who">${siteOf(r.site)} · needs craft ${r.level} · ${r.min}m</div>
...
S(
  ...
  `<p>You start at <b>level 1 in everything</b> and can reach <b>${SKILL_CAP}</b> in anything — your nature says how <i>gifted</i> you are towards a skill, never where you begin: levels are climbed, not handed out. The figures are different: their nature is banded <b>${NPC_MIN}–${NPC_MAX}</b> and they <i>sit</i> at it, so the best teacher alive can carry you to ${NPC_MAX}. Home ground lifts a figure in its biome's skill, but only to ${PLACE_CAP} — being born on the right land is not enough to make a master. <b>The last ${SKILL_CAP - NPC_MAX} levels are yours to grind</b> — you get them by doing the thing, and nobody can hand them to you. Every world has twelve masters, one per skill, pinned at ${NPC_MAX}: finding them is the game.</p>`,
...
sections.push(
  `<p class="muted" style="font-size:13px">A day is worth the seed minute plus one per discovered tile — from <code>${SEED_MIN}</code>, through ~<code>${ENERGY_START}</code> after home, up to <code>${WAKE_CAP}</code> (a day less its ${SLEEP_MIN / 60} hours of sleep). All numbers above are read live from the game's own constants.</p>`
)
...
const PILLARS = [
  ...
  { n: "Mind", emblem: "the word", hue: 262, about: "the self, its culture & design — know · mend · envision · make", skills: ["dream", "lore", "heal", "build"] }
]
...
// heal/dream swapped 2026-09-01 so each skill faces its opposite — see sim's WHEEL ORDER)
const WHEEL = ["scout", "travel", "gather", "craft", "hunt", "cook", "dream", "lore", "heal", "build", "farm", "trade"]
